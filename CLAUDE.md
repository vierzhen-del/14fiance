...
- **Sonnet 5 이하 · Haiku · 타사 모델(Gemini 등)**: 해당 스킬 문서를 **먼저 열어** 절차대로 수행하고,
  판단으로 단계를 건너뛰지 않는다.
- **모델과 무관하게 항상 유효**: 개인정보·보유자산 데이터 커밋 금지, 시크릿 노출 금지.
- **투자 판단·조언은 허용**(2026-08-02 사용자 확정, 종전 "매매 자문 금지" 규칙 폐지) — 비중·집중도·
  MDD·배당 지속성·NAV 침식 등을 근거와 함께 분석한다. 숫자와 근거를 제시하고 불확실한 건
  불확실하다고 말하되, 판단 자체를 회피하지 않는다. 최종 결정은 사용자 몫이다.
