...
/* A44: 🎯 ETF별 목표주식수 추적 — 종목(symbol) 단위로 저장하고, 그 종목을 보유·매수 중인 모든
   ...
   목표를 추가할 종목은 이미 월매수(monthlyQty>0)가 설정된 항목 중에서만 고르게 해
   오타로 존재하지 않는 종목을 목표로 잡는 실수를 막는다. */
function buildGoalTrackerHTML(perRow) {
  const goals = loadMyGoals();
  const buyPlanSymbols = [...new Map(
    perRow.filter((p) => p.monthlyQty > 0).map((p) => [p.symbol, p.meta ? p.meta.name : p.symbol])
  ).entries()];
  const optionsHTML = buyPlanSymbols
  ...
  const pickerHTML = `
    <div class="controls" style="margin-bottom:10px;">
      <select id="myGoalSymbolPick" aria-label="목표 추가할 종목 선택">
        <option value="">월매수 종목에서 선택…</option>
        ${optionsHTML}
      </select>
  ...
  if (!goals.length) {
    return pickerHTML + `<p class="compare-empty">${buyPlanSymbols.length ? "위에서 종목을 선택해 목표 주식수를 설정하세요." : "월매수 수량을 먼저 입력하면 목표로 고를 수 있는 종목이 여기 나타납니다."}</p>`;
  }
  ...
  const rows = goals.map((g) => {
    ...
    const divLabel = g.targetQty > 0 && priceKrw != null && effRate > 0
      ? `월 ${fmtW(g.targetQty * priceKrw * effRate / 12)} (연 ${(effRate * 100).toFixed(2)}%)`
