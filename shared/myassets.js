...
/* A74(2026-09-02 사용자 요청): 금융소득종합과세 도달률 — "주식 배당금만 기반해서 예측"한다는
   사용자 확정에 따라 이자소득 등 다른 금융소득은 합산하지 않는다(집계 자체가 taxableIncome
   인자 하나뿐인 이유). 2,000만원/1,000만원 버튼 전환 시 패널 전체를 다시 그리지 않고 이
   블록만 갱신할 수 있도록 별도 함수로 분리(myAssetTrendYearBtns 등과 같은 패턴). */
function buildFinIncomeBodyHTML(taxableIncome, threshold) {
  const fmtW = (v) => fmtPrice(v, "KRW");
...
async function renderMyAssets() {
  ...
  for (const it of items) {
    ...
    const taxRatioVal = it.taxRatio !== "" && it.taxRatio != null ? Number(it.taxRatio) / 100 : 1;
    ...
    perRow.push({ ...it, meta, close, isUsd, value, cost, profit, monthlyDiv, nextMonthDiv, monthlyBuy, erosion, usedConfirmed, trailReturn, trailDetail, effRate, divBasis, dpsFromRate, dpsGapPct, divPrice, divPriceDate, recordDate, recordFuture, buyDeadline, livePrevClose, taxRatioVal, taxableDiv, acctTaxType, isTaxDeferred, withheldTax, afterTaxMonthlyDiv, isTaxFreeEtf, annualMult });
  ...
  const taxSummaryHTML = `
    ...
    <div class="stat-row">
      ...
      <div class="stat">
        ...
        <p class="stat-sub">일반계좌만 15.4% · ISA는 한도 내 0원 가정</p>
      </div>
      <div class="stat">
    ...
    <div id="finIncomeBody">${buildFinIncomeBodyHTML(totalAnnualTaxableIncome, finThreshold)}</div>`;
  ...
  if (finIncomeThresholdBtns) {
    const renderFinIncome = () => {
      ...
      if (body) body.innerHTML = buildFinIncomeBodyHTML(totalAnnualTaxableIncome, threshold);
