...
async function renderMyAssets() {
  ...
  const REGION_ORDER = ["한국", "미국", "글로벌", "미분류"];
  const STYLE_ORDER = ["성장", "배당", "안전", "개별주", "미분류"];
  // A82(2026-09-07 사용자 요청 "종합/일반계좌ETF 구분 버튼 추가"): 시장·스타일·카테고리 비중을
  // "종합"(전체 계좌) 또는 "일반계좌ETF"(세금이연 계좌 제외·ETF만)로 필터링해 볼 수 있게 한다.
  // DC·연금·IRP는 세금이연 계좌라 당장 리밸런싱이 자유롭지 않으므로, 실제 처분 가능한 비중은
  // 일반계좌만 따로 봐야 한다는 요청 — accountTaxType()으로 계좌명을 일반화 판정(A52 원칙 유지).
  ...
  const isGeneralEtfRow = (p) => accountTaxType(p.account) === "general" && (!p.meta || p.meta.assetType !== "stock");
  const allocScope = state.myAllocScope === "generalEtf" ? "generalEtf" : "all";
  const allocScopeHTML = buildAllocCompositionHTML(allocScope === "generalEtf" ? perRow.filter(isGeneralEtfRow) : perRow);
  ...
  result.innerHTML = `
    ...
    <div class="dash-panel" data-tab="alloc" hidden>
      <div id="allocScopeBtns" class="seg" style="margin-bottom:8px;">
        <button type="button" data-scope="all">종합</button>
        <button type="button" data-scope="generalEtf">일반계좌ETF</button>
      </div>
  ...
  // A82(2026-09-07 사용자 요청 "종합/일반계좌ETF 구분 버튼 추가"): 비중분석 탭 종합/일반계좌ETF
  ...
  if (allocScopeBtns) {
    const renderAllocScope = () => {
      ...
      const scope = state.myAllocScope === "generalEtf" ? "generalEtf" : "all";
      if (body) body.innerHTML = buildAllocCompositionHTML(scope === "generalEtf" ? perRow.filter(isGeneralEtfRow) : perRow);
