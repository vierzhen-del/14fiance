...
function buildGoalTrackerHTML(perRow) {
  ...
  const rows = goals.map((g) => {
    ...
    const divLabel = g.targetQty > 0 && confirmedDps > 0
      ? `월 ${fmtW(g.targetQty * confirmedDps)} (확정 DPS ${confirmedDps.toLocaleString()}원/주 기준)`
      : g.targetQty > 0 && priceKrw != null && effRate > 0
