...
function buildReturnTrendHTML(dailyHistory, monthlyHistory, granularity, scope, scopeLabel) {
  ...
  const trRows = rows.map((r, i) => {
    ...
    return `<tr>
      <td>${r.key}</td>
      <td style="color:${r.g.pct >= 0 ? "var(--good)" : "var(--critical)"}">${r.g.pct >= 0 ? "+" : ""}${r.g.pct.toFixed(2)}%</td>
      <td style="color:${diffPt == null ? "var(--text-muted)" : diffPt >= 0 ? "var(--good)" : "var(--critical)"}">${diffPt == null ? "—" : `${diffPt >= 0 ? "+" : ""}${diffPt.toFixed(2)}%p`}</td>
      <td>${fmtW(r.g.costedValue)} / ${fmtW(r.g.cost)}</td>
    </tr>`;
  }).join("");
