...
const MY_ASSETS_GOALS_KEY = "my_assets_goals_v1"; // A44 🎯 종목별 목표주식수 추적([{symbol, targetQty}])
/* A90(2026-09-07 사용자 요청 "세금탭 신설 + 자사주 수령·특별배당 시뮬레이션"):
   ─ shareGrants: 앞으로 받을 자사주(RSU·성과급 주식 등) 수령 예정.
     [{symbol, account, month:"YYYY-MM", qty}] — 수령 시점부터 그 주식도 배당을 받으므로
     연도별 배당소득 추정에 반드시 들어가야 한다. 보유 종목행(qty)과 섞지 않는 이유:
     아직 받지 않은 주식이라 평가액·수익률에 들어가면 지금 자산이 부풀려진다.
   ─ specialDividends: 특별배당 "추정치". [{symbol, year, dps, recordMonth, note}]
     확정 공시가 아니라 언론·증권사 추정이므로 confirmedDps(실확)와 절대 섞지 않는다 —
     이 값은 오직 세금탭의 연도별 한도 시뮬레이션에서만 쓴다. */
const MY_SHARE_GRANTS_KEY = "my_share_grants_v1";
const MY_SPECIAL_DIV_KEY = "my_special_div_v1";
...
const GOAL_TRAIL_DEFAULT_MONTHS = 12;
...
/* 현재 폼(#myAssetRows)의 계좌별 보유 스냅샷 — Map(account -> Map(symbol -> {qty, monthlyQty, buyFreq, buyDay})).
   A44: 종전엔 qty(보유수량)만 담아서, 매수계획(월매수량·주기·요일)만 바뀐 갱신은 diffHoldingsSnapshots가
   전혀 감지하지 못해 변동이력에 아무 기록도 안 남았다(2026-08-11 실측: 월매수 8건 정정 반영해도
   변동이력 탭이 그대로였음). 매수계획도 같이 담아 diff 대상에 넣는다. */
function snapshotHoldingsMap() {
  ...
  document.querySelectorAll("#myAssetRows .portfolio-row").forEach((row) => {
    ...
    const monthlyQty = parseFloat(row.querySelector(".my-monthly").value) || 0;
    const buyFreq = row.querySelector(".my-buy-freq").value;
    const buyDay = row.querySelector(".my-buy-day").value.trim();
    if (!map.has(account)) map.set(account, new Map());
    const accMap = map.get(account);
    const prev = accMap.get(symbol);
    // qty는 종전처럼 같은 (계좌,종목) 중복행이면 합산, 매수계획은 마지막 행 값을 쓴다.
    accMap.set(symbol, { qty: (prev ? prev.qty : 0) + qty, monthlyQty, buyFreq, buyDay });
  });
  return map;
}

/* 두 매수계획이 같은지 비교 */
function samePlan(a, b) { return a.monthlyQty === b.monthlyQty && a.buyFreq === b.buyFreq && a.buyDay === b.buyDay; }

/* 두 시점의 보유 스냅샷을 비교해 종목변동(추가/삭제/수량변경/매수계획변경) 목록을 만든다 */
function diffHoldingsSnapshots(beforeMap, afterMap) {
  const changes = [];
  const accounts = new Set([...beforeMap.keys(), ...afterMap.keys()]);
  const empty = { qty: 0, monthlyQty: 0, buyFreq: "", buyDay: "" };
  for (const account of accounts) {
    ...
    for (const symbol of symbols) {
      const b = before.get(symbol) || empty;
      const a = after.get(symbol) || empty;
      if (b.qty !== a.qty) {
        const type = b.qty === 0 ? "added" : a.qty === 0 ? "removed" : "qty-changed";
        changes.push({ account, symbol, type, oldQty: b.qty, newQty: a.qty });
      }
      // A44: 보유수량은 그대로인데 매수계획만 바뀐 경우도 별도 항목으로 기록. qty도 신규/삭제된
      // 행이면(위에서 이미 added/removed로 기록됨) 매수계획 차이는 중복 기록하지 않는다.
      if (b.qty === a.qty && !samePlan(b, a) && (b.monthlyQty > 0 || a.monthlyQty > 0)) {
        changes.push({ account, symbol, type: "buyplan-changed", oldPlan: { ...b }, newPlan: { ...a } });
      }
    }
  }
  return changes;
...
/* 종목변동 표시용 상세 라벨 — qty 기반 변경은 "8→2058", 매수계획 변경은 "1주×매일→2주×매주/월요일" */
function changeDetailLabel(c) {
  if (c.type === "buyplan-changed") {
    const fmt = (p) => (p.monthlyQty > 0 ? `${p.monthlyQty}주×${p.buyDay ? `${p.buyFreq}/${p.buyDay}` : p.buyFreq}` : "—");
    return `${fmt(c.oldPlan)}→${fmt(c.newPlan)}`;
  }
  return `${c.oldQty}→${c.newQty}`;
}
...
/* A46: 수익률 스냅샷 — 일별·월별 자산 스냅샷에 매입단가 기준 수익률(총계좌·계좌별·비중별·국가별)을
   함께 기록해 "추이" 탭에서 수익률 변동 이력을 보여줄 수 있게 한다. 과거 시점의 매입단가는
   보존돼 있지 않으므로(현재값만 있음) 이 스냅샷을 쌓기 시작한 시점부터의 추이만 계산된다 —
   과거로 소급하지 않는다. */
function computeReturnBreakdown(csv) {
  if (!csv) return null;
  const pct = (cost, costedValue) => (cost > 0 ? ((costedValue - cost) / cost) * 100 : null);
  let totalCost = 0, totalCostedValue = 0;
  const byRegion = new Map(), byStyle = new Map();
  for (const p of csv.perRow) {
    if (!(p.value > 0)) continue;
    if (p.cost != null) { totalCost += p.cost; totalCostedValue += p.value; }
    const region = (p.meta && p.meta.region) || "미분류";
    const style = (p.meta && p.meta.style) || "미분류";
    for (const [map, key] of [[byRegion, region], [byStyle, style]]) {
      if (!map.has(key)) map.set(key, { cost: 0, costedValue: 0 });
      const g = map.get(key);
      if (p.cost != null) { g.cost += p.cost; g.costedValue += p.value; }
    }
  }
  const toObj = (map) => Object.fromEntries([...map.entries()].map(([k, g]) => [k, { cost: g.cost, costedValue: g.costedValue, pct: pct(g.cost, g.costedValue) }]));
  const byAccount = {};
  for (const [acc, g] of csv.accountMap) byAccount[acc] = { cost: g.cost, costedValue: g.costedValue, pct: pct(g.cost, g.costedValue) };
  return {
    total: { cost: totalCost, costedValue: totalCostedValue, pct: pct(totalCost, totalCostedValue) },
    byAccount, byRegion: toObj(byRegion), byStyle: toObj(byStyle),
  };
}
...
function serializeMyAssets() {
  return {
    ...
    // A90: 세금현황 탭의 연금 절세 넛지 계산용. 입력칸이 그 탭에서만(다시 그릴 때마다) 렌더되므로
    // DOM이 아니라 state를 원본으로 삼는다(myFinIncomeThreshold와 같은 패턴) — 입력 이벤트에서
    // state.myBirthYear를 갱신하고 saveMyAssets()를 호출한다.
    birthYear: state.myBirthYear || "",
    ...
    goals: (() => { try { return JSON.parse(localStorage.getItem(MY_ASSETS_GOALS_KEY) || "[]"); } catch (e) { return []; } })(),
...
/* A90: 두 목록은 화면 폼이 아니라 localStorage가 원본이다(세금탭에서만 편집).
   깨진 값이 들어와도 렌더가 멈추지 않도록 항상 배열로 정규화해서 돌려준다. */
/* symbol/month이 빈 값이면 걸러내지 않는다 — "+ 행 추가" 버튼이 채워 넣는 빈 행이 바로 그
   상태라, 여기서 거르면 사용자가 입력칸에 타이핑할 기회도 없이 방금 누른 행이 사라진다
   (2026-09-07 실측: 클릭해도 표가 안 늘어나는 것처럼 보이는 버그였음). 계산 쪽(qtyAt 등)은
   symbol/month가 비면 어차피 어느 해에도 안 걸리므로 안전하다 — 걸러야 할 건 배열이 아닌
   값·null 같은 완전히 깨진 항목뿐이다. */
function readShareGrants() {
  try {
    const v = JSON.parse(localStorage.getItem(MY_SHARE_GRANTS_KEY) || "[]");
    return Array.isArray(v) ? v.filter((g) => g && typeof g === "object") : [];
  } catch (e) { return []; }
}
function readSpecialDivs() {
  try {
    const v = JSON.parse(localStorage.getItem(MY_SPECIAL_DIV_KEY) || "[]");
...
function applyMyAssets(data) {
  ...
  if (Array.isArray(data.goals) && data.goals.length) localStorage.setItem(MY_ASSETS_GOALS_KEY, JSON.stringify(data.goals));
  /* A90: 자사주 수령·특별배당 추정도 복원한다. 다른 이력들과 달리 **빈 배열도 그대로 반영**한다 —
     "이 종목 자사주 계획 없어졌다"고 지워서 내보낸 파일을 가져왔는데 옛 목록이 되살아나면
  ...
  state.myBirthYear = data.birthYear || ""; // A90 — 세금현황 탭 렌더 시 입력칸에 채워진다
...
function buildReturnAnalysisHTML(regionRetMap, styleRetMap, accountMap, history, perRow) {
  ...
  if (history.length >= 2) {
    ...
    // A93(2026-09-11 사용자 요청 "BM 수익률대비 현 수익률"): 코스피 등락률을 나란히 본다.
    // A97(2026-09-21 사용자 "스샷값없어 미계산부분 확인"): 예전에는 **맨 처음·맨 끝** 스냅샷만
    // 봐서, 시작 스냅샷에 코스피가 없으면(코스피 저장 이전 기록) 뒤쪽 달에 값이 멀쩡히 있어도
    // 통째로 "비교 불가"가 됐다(2026-07 무 → 08·09 유 인데 비교 불가로 표시된 실사례).
    // 이제 **코스피 값이 있는 스냅샷들** 중 처음~끝 구간으로 비교하고, 내 수익률도 그 구간의
    // 평가액으로 다시 계산한다(구간이 다른 두 수익률을 빼면 초과수익이 거짓이 된다).
    // 구간이 전체와 다르면 화면에 기간을 함께 적는다 — 지어내지 않고 "어디까지 비교했는지"를 밝힌다.
    const kPts = sorted.filter((h) => h.kospi > 0 && h.value > 0);
    const kFirst = kPts[0], kLast = kPts[kPts.length - 1];
    const hasK = kPts.length >= 2 && kFirst.month !== kLast.month;
    const kospiPct = hasK ? ((kLast.kospi - kFirst.kospi) / kFirst.kospi) * 100 : null;
    const myKPct = hasK ? ((kLast.value - kFirst.value) / kFirst.value) * 100 : null;
    const alpha = hasK ? myKPct - kospiPct : null;
    const kSameSpan = hasK && kFirst.month === first.month && kLast.month === last.month;
    // A99: 비교에 쓴 코스피가 **그 달에 수집된 값이 아닐 수 있다**(예약 수집 지연). 그러면
    // 초과수익률도 그만큼 어긋나므로 조용히 넘기지 않고 어느 행이 언제 수집분인지 밝힌다.
    const kStale = hasK
      ? [kFirst, kLast].filter((h) => h.kospiAsOf && String(h.kospiAsOf).slice(0, 7) !== h.month)
          .map((h) => `${h.month} 행은 ${String(h.kospiAsOf).slice(0, 10)} 수집분`).join(" · ")
      : "";
    historyReturnHTML = `<div class="stat-row">
      ...
      <div class="stat">
        <p class="stat-label">코스피 대비 ${hasK ? (kSameSpan ? "(같은 기간)" : `(${kFirst.month} → ${kLast.month})`) : "(비교 불가)"}</p>
        <p class="stat-value" style="color:${!hasK ? "var(--text-muted)" : alpha >= 0 ? "var(--good)" : "var(--critical)"}">${!hasK ? "비교 불가" : `${alpha >= 0 ? "+" : ""}${alpha.toFixed(1)}%p`}</p>
        <p class="stat-sub">${!hasK
          ? `코스피 값이 함께 저장된 스냅샷이 2개월 이상 있어야 합니다(현재 ${kPts.length}개월).`
          : `내 ${myKPct >= 0 ? "+" : ""}${myKPct.toFixed(1)}% vs 코스피 ${kospiPct >= 0 ? "+" : ""}${kospiPct.toFixed(1)}%${kSameSpan ? "" : ` · 코스피 값이 있는 ${kFirst.month}~${kLast.month} 구간만 비교(내 수익률도 같은 구간으로 재계산)`}${kStale ? `<br><span style="color:#c26a00;">⚠ ${kStale} — 예약 수집이 지연돼 그 달 값이 아닙니다</span>` : ""}`}</p>
      </div>
    </div>
...
/* A90(2026-09-07 사용자 요청): 향후 5년 연도별 배당소득·종합과세 한도 시뮬레이션.
   "일반계좌 주식을 안 팔고 계속 보유"할 때, 자사주를 받아 수량이 늘고 특별배당까지 겹치면
   금융소득 종합과세 2,000만원 선을 언제 넘는가를 본다.

   왜 연도별인가 — 종합과세는 **연간 합산**이라 월 기준 화면(A74)으로는 특별배당처럼 한 해에
   한 번 크게 들어오는 돈을 볼 수 없다. 특별배당은 그 해에만 얹히고, 자사주는 받은 해부터
   영구히 수량을 늘린다 — 둘의 시간축이 달라서 연도별로 펼쳐야 판단이 된다.

   계산 원칙(지어내지 않기):
   - **일반계좌만** 분자에 넣는다. 연금·IRP·DC는 이연, ISA는 분리과세라 종합과세 대상이 아니다.
   - 과세표준은 taxRatio 실측값을 그대로 쓴다(미실측이면 100% 보수 가정 — A74와 동일 규칙).
   - 자사주는 **받는 달 이후의 기준일부터만** 배당을 받는다. 6월 수령분이 그해 1월 배당까지
     소급해 받는 것처럼 계산하면 첫해가 부풀려진다.
   - 특별배당은 **확정 공시가 아니라 추정**이므로 confirmedDps와 분리해 별도 입력으로 받고,
     화면에서도 "추정"으로 표시한다. 추정을 지우면 그 해 특별배당은 0으로 계산된다. */
function buildTaxProjectionHTML(perRow, grants, specials, threshold, scenario) {
  const fmtW = (v) => fmtPrice(v, "KRW");
  const thisYear = Number(todayStr().slice(0, 4));
  const years = [0, 1, 2, 3, 4, 5].map((i) => thisYear + i);

  // 일반계좌(=종합과세 대상) 보유분만 추린다
  const genRows = (perRow || []).filter((p) => p.acctTaxType === "general");
  const genSymbols = new Set(genRows.map((p) => p.symbol));

  // 자사주 수령분: 일반계좌로 들어오는 것만 종합과세 대상이 된다
  const genGrants = (grants || []).filter((g) => accountTaxType(g.account) === "general" && Number(g.qty) > 0);

  const nameOf = (sym) => {
    const hit = (perRow || []).find((p) => p.symbol === sym);
    if (hit && hit.meta) return hit.meta.name;
    const meta = state.metaBySymbol && state.metaBySymbol.get(sym);
    return meta ? meta.name : sym;
  };
  // 미실측이면 100% 가정(A74와 같은 규칙) — 자사주로 새로 들어오는 종목도 같은 기준
  const taxRatioOf = (sym) => {
    const hit = (perRow || []).find((p) => p.symbol === sym && p.taxRatioKnown);
    return hit ? hit.taxRatioVal : 1;
  };

  /* 그 해에 이 종목이 "일반계좌에서" 몇 주였는지 — 기준월(recordMonth)까지 받은 자사주를 더한다.
     지금 보유분은 이미 갖고 있으므로 항상 포함, 자사주는 수령월 <= 기준월인 것만 포함. */
  const qtyAt = (sym, year, recordMonth) => {
    let q = genRows.filter((p) => p.symbol === sym).reduce((s, p) => s + p.qty, 0);
    for (const g of genGrants) {
      if (g.symbol !== sym) continue;
      const gy = Number(String(g.month).slice(0, 4));
      const gm = Number(String(g.month).slice(5, 7));
      // qtyLow: 조건부 자사주의 "조건 미달 시" 수량(예: 성과조건 미충족으로 절반만 배정).
      // 시나리오 토글이 "low"면 그쪽을 쓴다 — 없으면 기본 qty를 그대로 쓴다.
      const useQty = scenario === "low" && g.qtyLow != null && g.qtyLow !== "" ? Number(g.qtyLow) : Number(g.qty);
      if (gy < year || (gy === year && gm <= recordMonth)) q += useQty;
    }
    return q;
  };

  // 시뮬레이션 대상 종목 = 지금 일반계좌 보유 + 앞으로 자사주로 들어올 종목
  const allSymbols = [...new Set([...genSymbols, ...genGrants.map((g) => g.symbol)])];
  const unregistered = []; // 배당이 등록 안 된 개별주 — 0으로 조용히 처리하지 않고 화면에 밝힌다

  const rows = years.map((year) => {
    let regular = 0, special = 0;
    const detail = [];
    for (const sym of allSymbols) {
      const ratio = taxRatioOf(sym);
      const sample = genRows.find((p) => p.symbol === sym);
      let symRegular = 0;
      if (sample) {
        /* A90 수정(2026-09-07 사용자 지적 "삼성전자 월배당 적용은 오류"):
           개별주(assetType==="stock")는 **분기배당·특별배당만** 받는다. 종전엔 payPeriod가
           "분기말"이 아니면 월배당 경로(monthlyDiv÷qty×12)로 빠졌는데, SK하이닉스처럼
           payPeriod가 비어 있는 개별주가 여기 걸리면 분기배당이 월배당으로 12배 계산된다.
           개별주는 분기 경로로 강제하고, 배당 미등록이면 **0으로 채우지 말고 목록에 남겨
           화면에 "배당 미등록"으로 밝힌다**(지어내지 않기). */
        const isStock = sample.meta && sample.meta.assetType === "stock";
        if (isStock) {
          if (sample.confirmedDps > 0) {
            for (const m of QUARTER_RECORD_MONTHS) symRegular += qtyAt(sym, year, m) * sample.confirmedDps;
          } else if (year === years[0] && !unregistered.some((u) => u.sym === sym)) {
            unregistered.push({ sym, name: nameOf(sym), qty: qtyAt(sym, year, 12) });
          }
        } else if (sample.payPeriod === "분기말") {
          for (const m of QUARTER_RECORD_MONTHS) symRegular += qtyAt(sym, year, m) * (sample.confirmedDps || 0);
        } else if (sample.monthlyDiv > 0) {
          // 월배당은 ETF만 — 12개월 각각의 시점 수량으로(자사주가 중간에 들어오면 그 달부터)
          const perShare = sample.qty > 0 ? sample.monthlyDiv / sample.qty : 0;
          for (let m = 1; m <= 12; m++) symRegular += qtyAt(sym, year, m) * perShare;
        }
      }
      // 특별배당(추정) — 그 해·그 종목에 등록된 것만
      let symSpecial = 0;
      for (const s of (specials || [])) {
        if (s.symbol !== sym || Number(s.year) !== year) continue;
        const rm = Number(s.recordMonth) || 12;
        symSpecial += qtyAt(sym, year, rm) * (Number(s.dps) || 0);
      }
      regular += symRegular * ratio;
      special += symSpecial * ratio;
      if (symRegular + symSpecial > 0) {
        detail.push({ sym, name: nameOf(sym), regular: symRegular, special: symSpecial, ratio, qty: qtyAt(sym, year, 12) });
      }
    }
    const total = regular + special;
    return { year, regular, special, total, pct: threshold > 0 ? (total / threshold) * 100 : 0, detail };
  });

  const yearRowsHTML = rows.map((r) => {
    const over = r.total > threshold;
    return `<tr>
      <td style="text-align:left;">${r.year}${r.year === thisYear ? " <span style=\"font-size:11px; color:var(--text-muted);\">(올해)</span>" : ""}</td>
      <td>${fmtW(r.regular)}</td>
      <td style="color:${r.special > 0 ? "var(--critical)" : "var(--text-muted)"}">${r.special > 0 ? fmtW(r.special) : "—"}</td>
      <td style="font-weight:700;">${fmtW(r.total)}</td>
      <td style="color:${over ? "var(--critical)" : r.pct >= 80 ? "var(--critical)" : "var(--good)"}; font-weight:700;">${r.pct.toFixed(1)}%</td>
      <td style="color:${over ? "var(--critical)" : "var(--good)"}">${over ? "초과 " + fmtW(r.total - threshold) : fmtW(threshold - r.total)}</td>
    </tr>`;
  }).join("");

  const firstOver = rows.find((r) => r.total > threshold);
  const peak = rows.reduce((a, b) => (b.total > a.total ? b : a), rows[0]);

  // 최대치 해의 종목별 내역 — "무엇이 한도를 밀어올리는지"를 바로 보게 한다
  const peakDetailHTML = peak.detail.sort((a, b) => (b.regular + b.special) - (a.regular + a.special)).map((d) => `<tr>
      <td style="text-align:left;">${d.name}<br><span style="font-size:11px; color:var(--text-muted);">${d.sym} · ${d.qty.toLocaleString()}주</span></td>
      <td>${fmtW(d.regular)}</td>
      <td style="color:${d.special > 0 ? "var(--critical)" : "var(--text-muted)"}">${d.special > 0 ? fmtW(d.special) : "—"}</td>
      <td>${(d.ratio * 100).toFixed(2)}%</td>
      <td style="font-weight:700;">${fmtW((d.regular + d.special) * d.ratio)}</td>
    </tr>`).join("");

  return `
    <div style="overflow-x:auto;">
    <table class="account-summary-table">
      <thead><tr><th>연도</th><th>정규배당</th><th>특별배당(추정)</th><th>과세대상 합계</th><th>도달률</th><th>${"여유/초과"}</th></tr></thead>
      <tbody>${yearRowsHTML}</tbody>
    </table>
    </div>
    <p class="stat-sub" style="margin-top:6px;">기준액 <b>${fmtW(threshold)}</b> 대비. <b>일반계좌 보유분만</b> 넣었습니다 — 연금저축·IRP·DC(이연)와 ISA(분리과세)는 종합과세 대상이 아닙니다. 과세표준 비율은 실측값을 쓰고, 미실측 종목은 보수적으로 100%로 잡습니다.</p>
    ${unregistered.length ? `<p class="stat-sub" style="color:var(--critical);">⚠️ 배당 미등록 개별주 ${unregistered.length}건은 이 시뮬레이션에서 <b>0원으로 빠졌습니다</b>(있는 배당을 지어내 채우지 않음) — 실제로는 이보다 더 걷힐 수 있습니다: ${unregistered.map((u) => `${u.name}(${u.sym}, ${u.qty.toLocaleString()}주)`).join(", ")}. 종목행에 confirmedDps·payPeriod를 등록하면 반영됩니다.</p>` : ""}
    ${firstOver
      ? `<p class="stat-sub" style="color:var(--critical); font-weight:700;">⚠️ ${firstOver.year}년에 기준액을 ${fmtW(firstOver.total - threshold)} 초과합니다 — 초과분이 다른 소득과 합산돼 누진과세(6.6~49.5%)됩니다.</p>`
      : `<p class="stat-sub" style="color:var(--good);">✅ 5년 내 기준액 초과 없음 — 최대치는 ${peak.year}년 ${fmtW(peak.total)}(도달률 ${peak.pct.toFixed(1)}%)입니다.</p>`}
    <p class="chart-title" style="margin-top:16px;">📊 ${peak.year}년(최대치) 종목별 내역</p>
    <div style="overflow-x:auto;">
    <table class="account-summary-table">
      <thead><tr><th>종목</th><th>정규배당</th><th>특별배당(추정)</th><th>과세표준비율</th><th>과세대상</th></tr></thead>
      <tbody>${peakDetailHTML || `<tr><td colspan="5" class="stat-sub">일반계좌 배당 종목이 없습니다.</td></tr>`}</tbody>
    </table>
    </div>`;
}

/* A89(2026-09-07 사용자 요청 "세금 부분 접기후 누르면 상세 과세부분이 표시"): 세금 블록을
   접어두고, 펼쳤을 때 "왜 이 금액이 나왔는지"를 계좌·종목 단위로 볼 수 있게 한다.

   계좌별 표가 먼저인 이유 — 원천징수가 계좌유형에서 갈리기 때문이다(일반계좌만 15.4%,
   DC·연금저축·IRP는 이연, ISA는 한도 내 0원). 종목별 표는 실제로 지금 세금이 붙는 일반계좌
   행만 담는다: 연금계좌 종목까지 넣으면 표만 길어지고 정작 "지금 걷히는 세금"은 안 보인다.
   과세표준 비율이 실측인지 100% 가정인지도 행마다 밝힌다 — A75에서 이 구분이 없어
   "종합과세 임박"으로 오인했던 전례가 있다. */
function buildTaxDetailHTML(perRow) {
  const fmtW = (v) => fmtPrice(v, "KRW");
  const rows = (perRow || []).filter((p) => p.monthlyDiv > 0);
  if (!rows.length) return `<p class="compare-empty">배당이 등록된 종목이 없어 과세 상세를 만들 수 없습니다.</p>`;

  const accMap = new Map();
  for (const p of rows) {
    const key = p.account || "계좌 미지정";
    if (!accMap.has(key)) accMap.set(key, { taxType: p.acctTaxType, div: 0, base: 0, tax: 0, after: 0 });
    const g = accMap.get(key);
    g.div += p.monthlyDiv; g.base += p.taxableDiv; g.tax += p.withheldTax; g.after += p.afterTaxMonthlyDiv;
  }
  const accHTML = [...accMap.entries()].sort((a, b) => b[1].div - a[1].div).map(([acc, g]) => {
    const info = ACCOUNT_TAX_INFO[g.taxType] || ACCOUNT_TAX_INFO.general;
    return `<tr>
      <td style="text-align:left;">${acc}</td>
      <td>${info.label}<br><span style="font-size:11px; color:var(--text-muted);">${info.tax}</span></td>
      <td>${fmtW(g.div)}</td>
      <td>${fmtW(g.base)}</td>
      <td style="color:${g.tax > 0 ? "var(--critical)" : "var(--text-muted)"}">${g.tax > 0 ? fmtW(g.tax) : "—"}</td>
      <td>${fmtW(g.after)}</td>
      <td style="color:${info.withdrawable ? "var(--good)" : "var(--text-muted)"}">${info.withdrawable ? "💰 가능" : "🔒 불가"}</td>
    </tr>`;
  }).join("");

  // 지금 실제로 원천징수가 걸리는 일반계좌 종목만 — 과세표준 비율의 출처(실측/가정)를 함께
  const genRows = rows.filter((p) => p.acctTaxType === "general").sort((a, b) => b.withheldTax - a.withheldTax);
  const symHTML = genRows.map((p) => `<tr>
      <td style="text-align:left;">${p.meta ? p.meta.name : p.symbol}<br><span style="font-size:11px; color:var(--text-muted);">${p.account || "계좌 미지정"}</span></td>
      <td>${fmtW(p.monthlyDiv)}</td>
      <td>${p.taxRatioKnown
        ? `${(p.taxRatioVal * 100).toFixed(2)}%<br><span style="font-size:11px; color:var(--good);">실측</span>`
        : `100%<br><span style="font-size:11px; color:var(--critical);">미실측·가정</span>`}</td>
      <td>${fmtW(p.taxableDiv)}</td>
      <td style="color:${p.withheldTax > 0 ? "var(--critical)" : "var(--text-muted)"}">${p.withheldTax > 0 ? fmtW(p.withheldTax) : "—"}</td>
    </tr>`).join("");

  return `
    <p class="chart-title" style="margin-top:4px;">🏦 계좌별 과세 상세</p>
    <div style="overflow-x:auto;">
    <table class="account-summary-table">
      <thead><tr><th>계좌</th><th>과세유형</th><th>월배당</th><th>과세표준</th><th>원천징수(지금)</th><th>세후</th><th>인출</th></tr></thead>
      <tbody>${accHTML}</tbody>
    </table>
    </div>
    <p class="stat-sub" style="margin-top:6px;">과세표준은 월배당 × 과세표준비율입니다 — 국내 커버드콜 ETF는 옵션프리미엄이 비과세 자본반환(ROC)으로 나가는 구간이 있어 배당금보다 과세표준이 작습니다. 노션 「배당기준 마스터」에서 실측한 값만 반영되고, 미실측 종목은 보수적으로 100%로 잡습니다.</p>
    ${genRows.length ? `
    <p class="chart-title" style="margin-top:16px;">🧾 일반계좌 종목별 과세 (지금 원천징수 대상)</p>
    <div style="overflow-x:auto;">
    <table class="account-summary-table">
      <thead><tr><th>종목</th><th>월배당</th><th>과세표준비율</th><th>과세표준</th><th>원천징수</th></tr></thead>
      <tbody>${symHTML}</tbody>
    </table>
    </div>
    <p class="stat-sub" style="margin-top:6px;">DC·연금저축·IRP는 배당 시점에 세금이 붙지 않아(이연) 이 표에서 뺐고, ISA는 한도 내 비과세로 가정해 원천징수가 0원입니다. <b>"미실측·가정"이 빨갛게 뜬 종목</b>은 실제보다 세금이 과대계상된 상태라, 노션에서 그 달 과세표준을 실측해 import JSON의 <code>taxRatio</code>에 넣으면 줄어듭니다.</p>` : ""}`;
}

/* A90(2026-09-07 사용자 요청 "연금저축 절세, DC IRP 절세방법 고려해서 연금개시 추천일 넛지"):
   연금계좌 세금은 배당(A74)과 축이 완전히 다르다 — 배당은 "지금 얼마 걷히나"이지만 연금은
   "언제 개시해서 몇 년째에 얼마씩 빼느냐"가 세율을 바꾼다. 두 트랙을 반드시 나눈다:
   - DC·IRP(퇴직급여, 이연) → 퇴직소득세 × 70%(1~10년차) / **60%(11년차~, 40% 감면)**.
     1,500만원 한도와 무관 — 분리과세라 상한 자체가 없다.
   - 연금저축(세액공제 재원) → 연금소득세 3.3~5.5%. **사적연금 합계 연 1,500만원**을 넘으면
     저율 분리과세가 깨지고 종합과세 또는 16.5% 분리과세 중 선택해야 한다.
   "세금미공제" 계좌(세액공제 안 받은 원금)는 애초에 인출 시 과세 대상이 아니라 별도로 뺀다.

   연금수령한도 = 평가액 ÷ (11 − 연차) × 120%, 11년차부터 한도 소멸(전액 인출 가능) — 국세청·
   미래에셋 자료 공통. 연차 기산은 "최초로 연금수령 할 수 있는 날이 속하는 과세기간"이 1년차이고
   실무상 금융기관에 **개시 신청**을 해야 그 시계가 돈다(2026-09-07 WebSearch로 확인, 소스는
   화면 캡션에 남긴다) — 그래서 "55세에 개시 신청 + 소액 인출"이 절세의 시작점이다.

   출생연도가 없으면 개시연도·11년차를 지어내지 않고 입력을 유도하는 안내만 낸다. */
function buildPensionNudgeHTML(accountMap, birthYear) {
  const fmtW = (v) => fmtPrice(v, "KRW");
  let dcIrpValue = 0, pensionValue = 0, taxFreeValue = 0;
  const dcIrpAccounts = [], pensionAccounts = [];
  for (const [acc, g] of accountMap) {
    const t = accountTaxType(acc);
    if (t === "dc" || t === "irp") { dcIrpValue += g.value; dcIrpAccounts.push(acc); }
    else if (t === "pension") {
      if (acc.includes("세금미공제")) taxFreeValue += g.value;
      else { pensionValue += g.value; pensionAccounts.push(acc); }
    }
  }
  if (dcIrpValue + pensionValue + taxFreeValue <= 0) {
    return `<p class="compare-empty">DC·IRP·연금저축 계좌 보유분이 없어 연금 넛지를 계산할 수 없습니다.</p>`;
  }

  const limitTable = (value, label) => {
    const rows = [1, 3, 5, 10].map((yr) => `<tr><td>${yr}년차</td><td>${fmtW(value / (11 - yr) * 1.2)}</td></tr>`).join("");
    return `<p class="stat-sub" style="margin-top:10px; font-weight:700;">${label} — 평가액 ${fmtW(value)}</p>
      <div style="overflow-x:auto;"><table class="account-summary-table" style="font-size:12.5px;">
        <thead><tr><th>연차</th><th>연금수령한도(그 해 최대 인출)</th></tr></thead>
        <tbody>${rows}<tr><td>11년차~</td><td style="color:var(--good); font-weight:700;">한도 없음(전액 가능)</td></tr></tbody>
      </table></div>`;
  };

  const by = Number(birthYear) > 1900 && Number(birthYear) < 2020 ? Number(birthYear) : null;
  let timelineHTML = `<p class="compare-empty">출생연도를 입력하면 55세 개시 신청 권장 연도와 11년차(퇴직소득세 40% 감면) 목돈 인출 권장 연도를 계산해 드립니다. 나이 없이 연차를 지어내지 않습니다.</p>`;
  if (by) {
    const age55Year = by + 55;
    const year11 = age55Year + 10; // 개시연도가 1년차이므로 11년차는 개시연도+10
    const thisYear = Number(todayStr().slice(0, 4));
    timelineHTML = `
      <div class="stat-row">
        <div class="stat">
          <p class="stat-label">📍 개시 신청 권장</p>
          <p class="stat-value">${age55Year}년${age55Year <= thisYear ? " (도달)" : ""}</p>
          <p class="stat-sub">만 55세. 이때 연금계좌에서 <b>최소액(예: 연 1만원)이라도 인출 신청</b>해야 연차 시계가 시작됩니다(신청 없이는 기산 안 됨).</p>
        </div>
        <div class="stat">
          <p class="stat-label">🎯 목돈 인출 권장</p>
          <p class="stat-value">${year11}년</p>
          <p class="stat-sub">11년차 — 퇴직소득세 <b>40% 감면</b> + 연금수령한도 소멸(전액 인출 가능)이 겹치는 시점입니다.</p>
        </div>
      </div>
      <p class="stat-sub" style="color:var(--critical); margin-top:6px;">⚠️ 연금을 개시하면 그만큼 DC·연금저축에서 돈이 빠져나가 <b>지금의 월배당 재투자 구조가 줄어듭니다</b> — 세금만 보고 결정하지 말고 「💰 월배당현황」의 계좌별 배당과 함께 판단하세요.</p>`;
  }

  const annualIfSplit10 = pensionValue / 10;
  const over1500 = annualIfSplit10 > 15_000_000;

  return `
    <p class="chart-title" style="margin-top:4px;">🏛️ 퇴직연금(DC·IRP) — 연차가 핵심</p>
    <p class="stat-sub">재원: ${dcIrpAccounts.join(", ") || "—"}. 이연된 <b>퇴직소득세</b>가 연차에 따라 70%(1~10년차) → <b>60%(11년차~, 40%감면)</b>로 줄어듭니다. 1,500만원 한도와는 무관합니다(분리과세).</p>
    ${limitTable(dcIrpValue, "DC+IRP 합계")}

    ${/* A91(2026-09-08 사용자 요청 "IRP 55세 만원 개시 필요 / DC 퇴직전 개시 불가 넛지 추가"):
         DC·IRP는 같은 "퇴직연금" 범주로 묶이지만 개시 가능 시점이 정반대라 계좌별로 갈라
         명시한다 — 이전 세션에서 사용자와 확인한 "재직 중 IRP는 55세부터 개시 가능하지만
         DC는 실제 퇴사 전엔 불가"를 정적 팁으로 박아둔다(나이 입력 없이도 항상 보임). */""}
    <div class="stat-row" style="margin-top:8px;">
      <div class="stat">
        <p class="stat-label">💡 IRP 절세팁</p>
        <p class="stat-sub"><b>55세부터 재직 중이라도 개시 가능</b>합니다. 이때 최소액(예: 연 1만원)이라도 인출 신청해야 연차 시계가 시작됩니다 — 개시 신청을 미루면 55세가 지나도 연차가 쌓이지 않습니다.</p>
      </div>
      <div class="stat">
        <p class="stat-label">⚠️ DC 주의사항</p>
        <p class="stat-sub">재직 중인 회사의 DC는 <b>실제 퇴사 전에는 연금개시가 불가</b>합니다 — 55세가 지나도 그 회사에 남아있는 한 인출 자체가 안 됩니다. 퇴사 시 DC 잔액이 이전될 IRP는 <b>이미 개시된 IRP와는 별도 계좌</b>여야 합니다(연금수령을 개시한 계좌엔 추가 납입 불가).</p>
      </div>
    </div>

    <p class="chart-title" style="margin-top:20px;">💳 연금저축 — 연 1,500만원이 핵심</p>
    <p class="stat-sub">재원: ${pensionAccounts.join(", ") || "—"}. <b>연금소득세 3.3~5.5%</b>(저율)이지만, 사적연금 수령액 합계가 연 1,500만원을 넘으면 저율과세가 깨지고 종합과세 또는 16.5% 분리과세 중 선택해야 합니다.</p>
    ${limitTable(pensionValue, "연금저축 합계")}
    <p class="stat-sub" style="margin-top:8px; color:${over1500 ? "var(--critical)" : "var(--good)"};">${over1500
      ? `⚠️ 평가액 ${fmtW(pensionValue)}을 10년에 걸쳐 균등 인출하면 연 ${fmtW(annualIfSplit10)}로 <b>1,500만원을 초과</b>합니다 — 더 오래(예: 11년 이상) 나눠 받아야 저율과세를 유지합니다.`
      : `✅ 평가액 ${fmtW(pensionValue)}을 10년에 걸쳐 균등 인출하면 연 ${fmtW(annualIfSplit10)}로 1,500만원 이내입니다.`}</p>

    ${taxFreeValue > 0 ? `<p class="chart-title" style="margin-top:20px;">🆓 세금미공제 연금저축 — 비과세</p>
    <p class="stat-sub">평가액 <b>${fmtW(taxFreeValue)}</b>은 세액공제를 받지 않은 원금이라 인출해도 과세 대상이 아닙니다. <b>가장 먼저 꺼내 써도 되는 돈</b>입니다.</p>` : ""}

    <p class="chart-title" style="margin-top:20px;">📅 개시 시점 넛지</p>
    <div class="controls" style="margin-bottom:8px;">
      <input type="number" id="myBirthYear" class="portfolio-weight" style="width:110px" min="1940" max="2015" placeholder="출생연도(예: 1975)" value="${birthYear || ""}">
    </div>
    ${timelineHTML}
    <p class="stat-sub" style="margin-top:8px; color:var(--text-muted);">근거: 연금수령연차 감면율(70%→60%)·1,500만원 분리과세 기준은 국세청·미래에셋투자와연금센터·삼일PwC 자료(2026-09-07 조회). 연차 기산이 개시 신청 시점에 좌우되는지는 실무 확인이 필요해 <b>가입 금융기관에 재확인을 권장</b>합니다.</p>`;
}

/* A90: 자사주 수령 예정·특별배당 추정 편집표. myContribRows(계좌별 월납입)와 같은 결의
   패턴 — 필드 하나 바뀔 때마다 배열 전체를 localStorage에 다시 쓰고 saveMyAssets()+
   renderMyAssets()로 세금현황 탭 전체를 다시 그린다(이 두 목록은 작아서 비용이 크지 않다).
   행 삭제는 즉시 반영, 행 추가는 빈 값 하나를 배열 끝에 붙인다. */
function buildShareGrantsHTML(grants) {
  const rows = (grants || []).map((g, i) => `<tr data-idx="${i}">
      <td><input type="text" class="grant-field portfolio-weight" style="width:110px" data-field="symbol" value="${g.symbol || ""}" placeholder="005930.KS"></td>
      <td><input type="text" class="grant-field portfolio-weight" style="width:120px" data-field="account" value="${g.account || ""}" placeholder="계좌명"></td>
      <td><input type="month" class="grant-field portfolio-weight" style="width:130px" data-field="month" value="${g.month || ""}"></td>
      <td><input type="number" class="grant-field portfolio-weight" style="width:80px" data-field="qty" value="${g.qty ?? ""}" placeholder="수량"></td>
      <td><input type="number" class="grant-field portfolio-weight" style="width:90px" data-field="qtyLow" value="${g.qtyLow ?? ""}" placeholder="조건미달시"></td>
      <td><button type="button" class="btn-action grant-del-btn" data-idx="${i}" style="padding:4px 8px; font-size:11px;">삭제</button></td>
    </tr>`).join("");
  return `<div style="overflow-x:auto;">
    <table class="account-summary-table" id="myGrantsTable">
      <thead><tr><th>종목코드</th><th>계좌</th><th>수령월</th><th>수량</th><th>조건미달시 수량(선택)</th><th></th></tr></thead>
      <tbody>${rows || `<tr><td colspan="6" class="stat-sub">등록된 자사주 수령 예정이 없습니다.</td></tr>`}</tbody>
    </table>
    </div>
    <div class="action-row" style="margin-top:6px;">
      <button type="button" id="myGrantAddBtn" class="btn-action">+ 행 추가</button>
    </div>
    <p class="stat-sub" style="margin-top:4px;">받는 달 이후 기준일부터만 그해 배당에 반영됩니다. "조건미달시 수량"을 채우면 아래 시나리오 토글로 두 경우를 비교할 수 있습니다.</p>`;
}
function buildSpecialDivHTML(specials) {
  const rows = (specials || []).map((s, i) => `<tr data-idx="${i}">
      <td><input type="text" class="special-field portfolio-weight" style="width:110px" data-field="symbol" value="${s.symbol || ""}" placeholder="005930.KS"></td>
      <td><input type="number" class="special-field portfolio-weight" style="width:80px" data-field="year" value="${s.year ?? ""}" placeholder="2026"></td>
      <td><input type="number" class="special-field portfolio-weight" style="width:90px" data-field="dps" value="${s.dps ?? ""}" placeholder="주당원"></td>
      <td><input type="number" class="special-field portfolio-weight" style="width:70px" data-field="recordMonth" value="${s.recordMonth ?? ""}" placeholder="기준월" min="1" max="12"></td>
      <td><input type="text" class="special-field portfolio-weight" style="width:160px" data-field="note" value="${s.note || ""}" placeholder="근거·출처"></td>
      <td><button type="button" class="btn-action special-del-btn" data-idx="${i}" style="padding:4px 8px; font-size:11px;">삭제</button></td>
    </tr>`).join("");
  return `<div style="overflow-x:auto;">
    <table class="account-summary-table" id="mySpecialDivTable">
      <thead><tr><th>종목코드</th><th>연도</th><th>주당 추정액</th><th>기준월</th><th>근거·출처</th><th></th></tr></thead>
      <tbody>${rows || `<tr><td colspan="6" class="stat-sub">등록된 특별배당 추정이 없습니다.</td></tr>`}</tbody>
    </table>
    </div>
    <div class="action-row" style="margin-top:6px;">
      <button type="button" id="mySpecialAddBtn" class="btn-action">+ 행 추가</button>
    </div>
    <p class="stat-sub" style="margin-top:4px;">확정 공시가 아니라 <b>추정치</b>입니다 — 기사·증권사 리포트 근거를 "근거·출처"에 남기고, 확정되면 이 표를 지우고 종목행의 <code>confirmedDps</code>(실확)로 옮기세요.</p>`;
}
...
function loadMyGoals() {
  try { return JSON.parse(localStorage.getItem(MY_ASSETS_GOALS_KEY) || "[]"); } catch (e) { return []; }
}
...
async function buildGoalTrackerHTML(perRow, latestRate) {
  ...
  const goalCalcs = goals.map((g) => {
    ...
    const meta = matched.length ? matched[0].meta : state.metaBySymbol.get(g.symbol);
    ...
    if (!(g.targetQty > 0)) {
      etaLabel = "목표 미입력";
    } else if (curQty >= g.targetQty) {
      etaLabel = "✅ 이미 도달";
    } else if (monthlyShares <= 0) {
      etaLabel = "⚠️ 월매수 미설정 — 예측 불가";
    } else {
      const monthsNeeded = Math.ceil((g.targetQty - curQty) / monthlyShares);
      const eta = new Date();
      eta.setMonth(eta.getMonth() + monthsNeeded);
      const y = Math.floor(monthsNeeded / 12), m = monthsNeeded % 12;
      const dur = [y > 0 ? `${y}년` : "", m > 0 ? `${m}개월` : ""].filter(Boolean).join(" ") || "1개월 미만";
      etaLabel = `${dur} 후 (${eta.getFullYear()}-${String(eta.getMonth() + 1).padStart(2, "0")} 예상)`;
    }

    // A44 수정(2026-08-11): divRate는 "월 분배율"이다(입력창 안내문 "월 분배율(%)" 참조,
...
/* A94b(2026-09-17 사용자 요청 "근거에 점도표·한미일 금리차 반영"): 한·미·일 정책금리.
   ⚠️ 이 앱은 금리 시계열을 수집하지 않는다(data/에는 가격만 있고 TLT·IEF는 채권 '가격'이지
   금리가 아니다). 그래서 과거 금리는 공개된 역사적 사실을 **연말 기준 근사치**로 상수화했다 —
   정책금리는 연 몇 회만 바뀌므로 연말값이면 사이클 국면을 읽는 데 충분하다는 판단이다.
   정확한 결정일·중간 변동은 담지 않으므로 화면에 "근사치"임을 밝히고, 매매 판단은 각
   중앙은행 발표를 직접 확인하도록 안내한다(없는 정밀도를 있는 척하지 않는다).
   미국은 목표범위 상단, 한국은 기준금리, 일본은 무담보콜 익일물 유도목표 기준. */
const POLICY_RATE_HISTORY = [
  // [연도, 미국(상단), 한국, 일본]
  [2006, 5.25, 4.50, 0.25], [2007, 4.25, 5.00, 0.50], [2008, 0.25, 3.00, 0.10],
  [2009, 0.25, 2.00, 0.10], [2010, 0.25, 2.50, 0.10], [2011, 0.25, 3.25, 0.10],
  [2012, 0.25, 2.75, 0.10], [2013, 0.25, 2.50, 0.10], [2014, 0.25, 2.00, 0.10],
  [2015, 0.50, 1.50, 0.10], [2016, 0.75, 1.25, -0.10], [2017, 1.50, 1.50, -0.10],
  [2018, 2.50, 1.75, -0.10], [2019, 1.75, 1.25, -0.10], [2020, 0.25, 0.50, -0.10],
  [2021, 0.25, 1.00, -0.10], [2022, 4.50, 3.25, -0.10], [2023, 5.50, 3.50, -0.10],
  [2024, 4.50, 3.00, 0.25], [2025, 3.75, 2.50, 0.75], [2026, 4.00, 3.00, 1.00],
];

/* 현재 국면 — 2026-09 시점 확인값. 세 나라가 **동시에 인상**하는 드문 국면이라 이 자체가
   보수 시나리오의 근거다(할인율이 전 세계적으로 올라간다). */
const POLICY_RATE_NOW = {
  asOf: "2026-09-17",
  us: { label: "미국", rate: 3.875, band: "3.75~4.00%", note: "2026-09-16 FOMC 25bp 인상 — 2023년 이후 첫 인상. 18명 중 16명이 연내 추가 1회 이상 가능성" },
  kr: { label: "한국", rate: 3.00, band: "3.00%", note: "2.50%→2.75%→3.00% 두 차례 연속 인상" },
  jp: { label: "일본", rate: 1.00, band: "약 1.00%", note: "1995년 이후 최고. 2026-09-17 회의 추가 인상 확률 61%(시장 반영치)" },
...
const RATE_LINE_DEFS = {
  ...
  kr: { label: "한국 기준금리(우축)", col: 2, color: "#c2348b" }, // 코스피200 선(초록)과 겹치지 않게 자홍
...
const RATE_FOCUS_DEFS = {
  ...
  kr: {
    ...
    note: "한국 기준금리(자홍 점선·우축)와 코스피200(좌축)입니다. 한국은 <b>한·미 금리차가 역전되면</b> 원화 약세·외국인 자금유출 압력이 겹쳐, 금리를 올려도 지수가 함께 눌리는 구간이 있었습니다.",
  },
  jp: {
...
/* A95b(2026-09-17 사용자 "년도를 전체/최근10년/5년/1년 줌인 / x축 기간이 넓어 조정 필요"):
   20년을 한 화면에 그리면 최근 몇 년의 움직임이 왼쪽 20년에 눌려 안 보인다. 기간 버튼으로
   끝(최신)에서 거꾸로 N년만 잘라 본다. 자른 구간의 **시작을 0%로 다시 맞추므로** "최근 1년
   수익률"을 바로 읽을 수 있다(전체 구간 누적수익률을 그대로 잘라 보여주면 시작점이 +500%
   같은 값이라 기울기만 보이고 수익률은 못 읽는다). */
const RATE_RANGE_DEFS = {
  all: { label: "전체", years: 0 },
  y10: { label: "최근 10년", years: 10 },
  y5: { label: "5년", years: 5 },
  y1: { label: "1년", years: 1 },
};

/* 구간 자르기 — 경계에 걸친 선은 잘린 위치의 값(선형 보간)을 시작점으로 둔다. 없는 점을
   만들어내는 게 아니라 **이미 그려지던 선을 창 경계에서 자르는 것**이다(자르지 않고 직전
   점을 그대로 살리면 그 계열만 창보다 앞에서 시작해 x축이 어긋난다).
   정책금리는 연말 기준 근사치라 경계값도 앞뒤 연말값을 잇는 선에서 읽은 값 — 화면에 밝힌다. */
function clipSeriesFrom(s, startDate) {
  const i = s.dates.findIndex((d) => d >= startDate);
  if (i < 0) return null;
  const dates = s.dates.slice(i), values = s.values.slice(i);
  if (i > 0 && dates[0] > startDate) {
    const t0 = Date.parse(s.dates[i - 1]), t1 = Date.parse(s.dates[i]), t = Date.parse(startDate);
    const f = t1 > t0 ? (t - t0) / (t1 - t0) : 0;
    dates.unshift(startDate);
    values.unshift(s.values[i - 1] + (s.values[i] - s.values[i - 1]) * f);
  }
  return dates.length >= 2 ? { ...s, dates, values } : null;
}
...
/* 실제가 두 선 대비 어디인지 → 구간·색·행동 가이드·현금 비중.
   ...
   A94b: cash = 그 구간에서 **들고 있을 현금(대기자금) 비중** 권고. 배당으로 생활비를 쓰는
...
/* 금리 국면별 포트폴리오 비중 가이드 — 미국/국내를 나눠 본다. 지금 국면(한·미·일 동시 인상)은
   POLICY_RATE_NOW로 판정하지 않고 사용자가 표에서 직접 읽게 둔다: "지금이 인상기인가"는
   중앙은행 발표를 봐야 하는 판단이고, 앱이 단정하면 틀렸을 때 손해가 사용자에게 간다. */
function buildRateCyclePortfolioHTML() {
  const rows = [
    ["금리 인상기", "미국", "성장·고밸류 비중 축소, 현금성(초단기채·MMF) 확대. 장기채(TLT 등) 듀레이션 리스크 주의 — 금리가 더 오르면 가격이 눌린다. 배당은 <b>이익으로 주는</b> 종목 위주."],
    ["금리 인상기", "국내", "커버드콜 분배금은 기초지수가 빠지면 같이 준다(기준가×분배율) — '분배율 높음=안전' 오해 금지. 은행·보험 등 금리 수혜 섹터는 상대적 방어. 예적금·파킹 금리 재점검."],
    ["금리 인하기", "미국", "장기채·리츠의 가격 회복 여력이 커진다. 성장주는 할인율 부담이 줄어 유리. 단 <b>'경기방어용 인하'인지 '예방적 인하'인지</b>가 주가 반응을 가른다 — 인하 자체보다 이유를 볼 것."],
    ["금리 인하기", "국내", "만기 도래 예적금의 재예치 금리가 낮아지므로 자금 재배치 계획을 미리. 환율(원화 약세 시 해외자산 원화환산 이익)도 함께 점검."],
  ];
  return `<div style="overflow-x:auto;"><table class="account-summary-table" style="font-size:12.5px;">
      <thead><tr><th>국면</th><th>시장</th><th>비중 가이드</th></tr></thead>
      <tbody>${rows.map(([c, m, g]) => `<tr><td>${c}</td><td>${m}</td><td>${g}</td></tr>`).join("")}</tbody>
    </table></div>
    <p class="stat-sub" style="margin-top:6px;">지금이 어느 국면인지는 <b>앱이 단정하지 않습니다</b> — FOMC·한국은행 발표를 직접 확인하세요. 위 표 바로 아래 "현재 정책금리"는 2026-09 시점 확인값이며, 세 나라가 동시에 올리는 드문 국면입니다.</p>`;
}
...
function buildIndexOutlookHTML(liveGlobal) {
  ...
  const guideRows = INDEX_OUTLOOK_DEFS.map((d) => {
    ...
    const band = actual ? indexOutlookBand(actual, lv.cons, lv.opt) : null;
    if (!band) return "";
    return `<tr><td style="color:${band.color};">${band.icon} ${d.label}<br><span class="stat-sub" style="font-size:11px;">${band.zone}</span></td><td style="white-space:nowrap;">${band.cash}</td><td>${band.guide}</td></tr>`;
  }).join("");
  ...
  return `<div class="stat-row">${nowCards}</div>
    ...
    <div style="overflow-x:auto;"><table class="account-summary-table">
      <thead><tr><th>구간</th><th>현금<br>비중</th><th>가이드</th></tr></thead><tbody>${guideRows}</tbody>
    </table></div>
    <p class="stat-sub" style="margin-top:6px;">구간 경계(보수선 -15% / 희망선 +15%)는 "한 해 수익률 한 번치" 정도로 잡은 실무 기준이지 통계로 검증된 값이 아닙니다 — 매매 신호가 아니라 <b>점검 트리거</b>로 쓰세요. 현금 비중은 <b>총자산 대비 대기자금</b> 기준이며, 배당으로 생활비를 쓰는 구조에서는 현금이 0이면 하락장에 자산을 팔아야 하므로 과열일수록 늘려 두는 쪽으로 잡았습니다.</p>` : ""}
    ...
    <p class="chart-title" style="margin-top:20px;">🏛️ 현재 정책금리와 금리차 (${POLICY_RATE_NOW.asOf} 확인)</p>
    <div class="stat-row">
      ${["us", "kr", "jp"].map((k) => {
        const r = POLICY_RATE_NOW[k];
        return `<div class="stat">
          <p class="stat-label">${r.label}</p>
          <p class="stat-value">${r.band}</p>
          <p class="stat-sub">${r.note}</p>
        </div>`;
      }).join("")}
    </div>
    <div class="stat-row" style="margin-top:8px;">
      <div class="stat">
        <p class="stat-label">한·미 금리차</p>
        <p class="stat-value" style="color:${POLICY_RATE_NOW.kr.rate < POLICY_RATE_NOW.us.rate ? "var(--critical)" : "var(--good)"};">${(POLICY_RATE_NOW.kr.rate - POLICY_RATE_NOW.us.rate).toFixed(2)}%p</p>
        <p class="stat-sub">한국이 ${POLICY_RATE_NOW.kr.rate < POLICY_RATE_NOW.us.rate ? "낮음(역전) — 원화 약세·외국인 자금유출 압력 요인" : "높음 — 원화 강세 요인"}</p>
      </div>
      <div class="stat">
        <p class="stat-label">미·일 금리차</p>
        <p class="stat-value">${(POLICY_RATE_NOW.us.rate - POLICY_RATE_NOW.jp.rate).toFixed(2)}%p</p>
        <p class="stat-sub">엔캐리 트레이드의 연료 — 일본이 올리고 미국이 내려 차가 좁혀지면 엔캐리 청산(글로벌 자산 매도) 압력</p>
      </div>
    </div>
    <details class="collapse-box" style="margin-top:10px;">
      <summary>🔮 FOMC 점도표 경로 (2026-09 중앙값)</summary>
      <div class="collapse-body">
        <div style="overflow-x:auto;"><table class="account-summary-table" style="font-size:12.5px;">
          <thead><tr><th>시점</th><th>정책금리 중앙값</th></tr></thead>
          <tbody>${POLICY_RATE_NOW.dots.map(([w, v]) => `<tr><td>${w}</td><td>${v}</td></tr>`).join("")}</tbody>
        </table></div>
        <p class="stat-sub" style="margin-top:6px;">장기중립금리를 3.06%→3.25%로 올려 잡았다는 건 <b>팬데믹 이전 저금리로 돌아갈 여지가 줄었다</b>는 뜻입니다 — 할인율이 지난 10년 평균보다 높게 유지되므로 보수 시나리오의 근거가 됩니다.</p>
      </div>
    </details>
    ...
    <div id="myRateRangeBtns" class="seg" style="margin-bottom:8px;">
      ${Object.entries(RATE_RANGE_DEFS).map(([k, d]) => `<button type="button" data-range="${k}">${d.label}</button>`).join("")}
    </div>
    ...
    <p class="chart-title" style="margin-top:20px;">🧰 금리 국면별 포트폴리오 비중 가이드</p>
    ${buildRateCyclePortfolioHTML()}
...
function renderOutlookPastComboChart() {
  ...
  const range = RATE_RANGE_DEFS[state.myRateRange] ? state.myRateRange : "all";
  const rangeDef = RATE_RANGE_DEFS[range];
  ...
  const rangeBtns = document.getElementById("myRateRangeBtns");
  if (rangeBtns) rangeBtns.querySelectorAll("button").forEach((b) => b.classList.toggle("active", b.dataset.range === range));
  const note = document.getElementById("myRateFocusNote");
  if (note) {
    note.innerHTML = def.note + (rangeDef.years
      ? ` <b>최근 ${rangeDef.years}년</b> 구간만 잘라 보고 있습니다 — 지수선은 <b>구간 시작을 0%로 다시 맞춰</b> 그 기간 수익률을 바로 읽을 수 있고, 금리선은 연말 기준 근사치라 구간 시작점 값은 앞뒤 연말값을 잇는 선에서 읽은 값입니다${rangeDef.years <= 1 ? " (1년 구간에는 연말 점이 한 개뿐이라 금리선이 거의 직선으로 보입니다)" : ""}.`
      : "");
  }
  ...
  // 기간 줌 — 가장 최신 점에서 거꾸로 N년. 자른 뒤 지수(좌축)만 구간 시작 0%로 재기준한다
  // (금리는 절대 수준 자체가 의미라 손대지 않는다).
  let shown = series;
  if (rangeDef.years > 0) {
    const endTs = Math.max(...series.map((s) => Date.parse(s.dates[s.dates.length - 1])));
    const start = new Date(endTs);
    start.setUTCFullYear(start.getUTCFullYear() - rangeDef.years);
    const startStr = start.toISOString().slice(0, 10);
    shown = series.map((s) => clipSeriesFrom(s, startStr)).filter(Boolean)
      .map((s) => {
        if (s.axis === "right") return s;
        const f = 1 + s.values[0];
        return { ...s, values: s.values.map((v) => (1 + v) / f - 1) };
      });
    if (!shown.length) { el.innerHTML = `<p class="compare-empty">이 기간에 표시할 데이터가 없습니다.</p>`; return; }
  }
  el.innerHTML = "";
  buildCompareChart(el, shown, {
...
/* 향후 전망 경로 + 실제 기록 차트 — 앵커를 0%로 정규화해 여러 지수를 한 차트에서 비교한다
   (지수 레벨이 6천~2만6천으로 달라 그대로 그리면 코스피가 바닥에 깔린다).
   S&P500·코스피만 그린다: 나스닥까지 넣으면 선이 6개가 돼 모바일 폭에서 읽을 수 없다
   (나스닥 수치는 위의 카드·경로표에서 확인). 기록 버튼이 새 점을 찍은 뒤에도 이 함수만
   다시 불러 전체 재렌더 없이 실제 선을 갱신한다. */
function renderOutlookFutureChart() {
  const futEl = document.getElementById("myOutlookFutureChart");
  if (!futEl) return;
  const anchorYear = Number(INDEX_OUTLOOK_ANCHOR_DATE.slice(0, 4));
  const futDates = [];
  for (let y = anchorYear; y <= anchorYear + 10; y++) {
    futDates.push(y === anchorYear ? INDEX_OUTLOOK_ANCHOR_DATE : `${y}-12-31`);
  }
  const mk = (def, kind, color) => ({
    label: `${def.label} ${kind === "cons" ? "보수" : "희망"}`, color, dash: true,
  ...
  for (const [def, color] of [[sp, "#0b3d91"], [ks, "#0c6b3f"]]) {
    const pts = log.filter((r) => r[def.key] && r[def.key].actual > 0);
    if (pts.length >= 2) {
      series.push({
        label: `${def.label} 실제`, color,
        dates: pts.map((r) => r.date), values: pts.map((r) => r[def.key].actual / def.base - 1),
      });
    }
  }
  futEl.innerHTML = "";
  buildCompareChart(futEl, series, { anchorZero: true });
}
...
/* A99(2026-09-21 사용자 "추이 코스피반영 오류체크"): 스냅샷의 코스피가 **그날 값이 아닐 수
   있다** — 앱은 live-trading 브랜치의 latest_global.json을 읽는데, GitHub 예약 수집은
   best-effort라 지연·누락되면 며칠 전 값을 그대로 물고 있는다(실사례: 9/21 월요일 낮에
   대시보드는 네이버 실시간 7,018.92인데 스냅샷에는 9/19 수집분 6,894.23이 찍혔다).
   A61에서 수집 시각을 저장해 뒀지만 title 속성(마우스 호버)뿐이라 **모바일에서는 볼 수가
   없었다** — 수집일이 그 행의 날짜와 다르면 숫자 밑에 눈에 보이게 표기한다. */
function kospiCellHTML(kospi, asOf, rowKey) {
  if (!(kospi > 0)) return `<td>—</td>`;
  const num = kospi.toLocaleString(undefined, { maximumFractionDigits: 2 });
  const asOfDate = asOf ? String(asOf).slice(0, 10) : "";
  // rowKey가 "YYYY-MM-DD"(일별)면 날짜까지, "YYYY-MM"(월별)이면 월까지 같으면 정상으로 본다.
  const same = !asOfDate || asOfDate.slice(0, rowKey.length) === rowKey;
  const title = asOf ? ` title="${asOf} 수집분 — 예약 수집(30분 주기)이 지연되면 이전 값이 그대로 남는다"` : "";
  if (same) return `<td${title}>${num}</td>`;
  return `<td${title}>${num}<br><span class="stat-sub" style="font-size:11px; color:#c26a00;">⚠ ${asOfDate.slice(5).replace("-", "/")} 수집분</span></td>`;
}
...
function buildDailyAssetHTML(dailyHistory) {
  ...
  const rows = sorted.map((h, i) => {
    ...
    return `<tr>
      ...
      ${kospiCellHTML(h.kospi, h.kospiAsOf, h.date)}
...
function buildMonthlyAssetHTML(monthlyHistory) {
  ...
  const rows = sorted.map((h, i) => {
    ...
    return `<tr>
      ...
      <td style="color:${diff == null ? "var(--text-muted)" : diff >= 0 ? "var(--good)" : "var(--critical)"}">${diff == null ? "—" : `${diff >= 0 ? "+" : ""}${fmtW(diff)} (${pct.toFixed(2)}%)`}</td>
      <td>${h.monthlyDiv > 0 ? fmtW(h.monthlyDiv) : "—"}</td>
      ${kospiCellHTML(h.kospi, h.kospiAsOf, h.month)}
    </tr>`;
  }).join("");
  return `<div style="overflow-x:auto;">
    <table class="account-summary-table">
...
/* A46: 수익률 추이 — scope는 "total" | 계좌명 | 성향명 | 지역명. entries의 returnBreakdown이
   없는 과거 스냅샷(이 기능 추가 이전 기록)은 건너뛴다 — 없는 값을 0으로 지어내지 않는다. */
function returnTrendRows(entries, keyField, scope) {
  const pick = (rb) => {
    if (!rb) return null;
    if (scope === "total") return rb.total;
    return rb.byAccount[scope] || rb.byRegion[scope] || rb.byStyle[scope] || null;
  };
  return entries
    .map((h) => ({ key: h[keyField], g: pick(h.returnBreakdown) }))
    .filter((r) => r.g && r.g.pct != null);
}

function buildReturnTrendHTML(dailyHistory, monthlyHistory, granularity, scope, scopeLabel) {
  const fmtW = (v) => fmtPrice(v, "KRW");
  let rows;
  if (granularity === "daily") {
    rows = returnTrendRows(dailyHistory.slice().sort((a, b) => a.date.localeCompare(b.date)), "date", scope);
  } else if (granularity === "monthly") {
    rows = returnTrendRows(monthlyHistory.slice().sort((a, b) => a.month.localeCompare(b.month)), "month", scope);
  } else {
    // 연도별: 월별 스냅샷을 연도로 묶어 그 해의 마지막 기록만 사용
    const byYear = new Map();
    for (const h of monthlyHistory.slice().sort((a, b) => a.month.localeCompare(b.month))) byYear.set(h.month.slice(0, 4), h);
    rows = returnTrendRows([...byYear.values()], "month", scope).map((r) => ({ ...r, key: r.key.slice(0, 4) }));
  }
  if (!rows.length) {
    return `<p class="compare-empty">${scopeLabel} 수익률 스냅샷이 아직 없습니다 — "오늘 자산 스냅샷"·"이번 달 스냅샷 저장"을 누르면 그 시점의 매입단가 기준 수익률이 함께 기록되고, 이후부터 추이가 쌓입니다(과거로 소급되진 않습니다).</p>`;
  }
  // A83(2026-09-07 사용자 요청 "최신일자 먼저 정렬"): 전기간 대비(diffPt)는 시간순(오래된→최신)
  // 기준 이전 값과 비교해야 맞으므로 그 계산은 그대로 두고, 화면 표시 순서만 뒤집어 최신이
  // 맨 위로 오게 한다.
  const trRows = rows.map((r, i) => {
    const prev = rows[i - 1];
    const diffPt = prev ? r.g.pct - prev.g.pct : null;
    return { key: r.key, pct: r.g.pct, diffPt, costedValue: r.g.costedValue, cost: r.g.cost };
  }).reverse().map((r) => `<tr>
      <td>${r.key}</td>
      <td style="color:${r.pct >= 0 ? "var(--good)" : "var(--critical)"}">${r.pct >= 0 ? "+" : ""}${r.pct.toFixed(2)}%</td>
      <td style="color:${r.diffPt == null ? "var(--text-muted)" : r.diffPt >= 0 ? "var(--good)" : "var(--critical)"}">${r.diffPt == null ? "—" : `${r.diffPt >= 0 ? "+" : ""}${r.diffPt.toFixed(2)}%p`}</td>
      <td>${fmtW(r.costedValue)} / ${fmtW(r.cost)}</td>
    </tr>`).join("");
  const periodLabel = granularity === "daily" ? "날짜" : granularity === "monthly" ? "월" : "연도";
  return `<div style="overflow-x:auto;">
    <table class="account-summary-table">
      <thead><tr><th>${periodLabel}</th><th>수익률</th><th>전기간 대비</th><th>평가액 / 매입원가</th></tr></thead>
      <tbody>${trRows}</tbody>
    </table>
    </div>
...
const CHANGE_TYPE_LABEL = { added: "🆕 신규", removed: "🗑️ 삭제", "qty-changed": "🔁 수량변경", "buyplan-changed": "📅 매수계획변경" };
...
function changeItemsHTML(changeList) {
  ...
  return [...byAccount.entries()].map(([acc, list]) => {
    const li = (c) => `<li>${changeSymbolLabel(c.symbol)} <b>${CHANGE_TYPE_LABEL[c.type] || c.type}</b> <span class="chg-qty">${changeDetailLabel(c)}</span></li>`;
...
const CHANGE_SOURCE_LABEL = { "capture-account": "📸 계좌 캡처", "capture-buyplan": "📈 월매수 캡처", "capture-account-reset": "🆕 완전 신규 업데이트", "capture-buyplan-reset": "🆕 매수계획 전체 재설정", import: "📂 가져오기", "import-csv": "📄 CSV/TSV 가져오기" };
...
/* ---------- A100(2026-09-21 사용자 "지금확인 버튼 누를때 업데이트 실행되게") ----------
   종전 "⏱️ 지금 확인"은 **이미 수집돼 있는 JSON을 다시 읽기만** 했다. 예약 수집이 멈춰 있으면
   몇 번을 눌러도 같은 과거 값이 나왔다(A99에서 그 사실이 보이게는 했지만, 고치려면 여전히
   PC에서 수동 실행해야 했다). 이제 기기에 GitHub 토큰이 저장돼 있으면 이 버튼이 수집
   워크플로를 **직접 실행**시키고, 새 결과가 올라올 때까지 기다렸다가 화면을 갱신한다.

   토큰은 이 기기 localStorage에만 둔다(KIS 앱키·텔레그램 토큰과 같은 취급) — 내보내기 JSON·
   웹훅·저장소 어디에도 싣지 않는다. 필요한 권한은 14fiance 한 곳의 Actions: Read and write뿐.

   갱신 확인은 raw.githubusercontent가 아니라 GitHub API로 읽는다 — raw는 CDN이 최대 5분
   캐시하고 **쿼리스트링을 붙여도 우회되지 않는 것을 실측**했다(방금 올라온 커밋이 안 보임). */
const MY_GH_PAT_KEY = "my_gh_dispatch_pat_v1";
const GH_DISPATCH_REPO = "vierzhen-del/14fiance";
const GH_DISPATCH_REF = "claude/us-etf-mdd-calculator-gdwui7";
const GH_DISPATCH_TARGETS = [
  { wf: "intraday-kr.yml", path: "latest_kr.json", branch: "live", key: "liveKr", label: "국내 시세", needsLiveToggle: true },
  { wf: "intraday-global.yml", path: "latest_global.json", branch: "live-trading", key: "liveGlobal", label: "지수(코스피 등)" },
];
const GH_DISPATCH_WAIT_MS = 180000; // 국내 수집이 보통 1분 30초쯤 걸린다(실측 06:35:02→06:36:49)
const GH_DISPATCH_POLL_MS = 8000;
const GH_DISPATCH_FRESH_MIN = 3; // 이 분(分) 안에 수집된 값이면 또 실행하지 않는다(연타 방지)

function ghDispatchPat() { try { return localStorage.getItem(MY_GH_PAT_KEY) || ""; } catch (e) { return ""; } }
function sleepMs(ms) { return new Promise((res) => setTimeout(res, ms)); }

async function ghApiFetch(path, opts = {}) {
  return fetch(`https://api.github.com/repos/${GH_DISPATCH_REPO}${path}`, {
    ...opts,
    cache: "no-store",
    headers: { Authorization: `Bearer ${ghDispatchPat()}`, "X-GitHub-Api-Version": "2022-11-28", ...(opts.headers || {}) },
    signal: AbortSignal.timeout(15000),
  });
}

async function ghReadLiveFile(t) {
  const res = await ghApiFetch(`/contents/${t.path}?ref=${t.branch}&t=${Date.now()}`,
    { headers: { Accept: "application/vnd.github.raw+json" } });
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  return res.json();
}

async function ghDispatchWorkflow(wf) {
  const res = await ghApiFetch(`/actions/workflows/${wf}/dispatches`, {
    method: "POST",
...
/* "⏱️ 지금 확인" 공통 동작 — 헤더·히트맵 탭 두 버튼이 같이 쓴다.
   토큰이 없으면 종전처럼 캐시만 비우고 다시 읽는다(기능이 사라지지 않게). */
async function refreshLiveQuotesNow(btnId, statusId) {
  const btn = btnId ? document.getElementById(btnId) : null;
  const setMsg = (m) => { const el = statusId ? document.getElementById(statusId) : null; if (el) el.textContent = m; };
  if (btn) btn.disabled = true;
  const targets = GH_DISPATCH_TARGETS.filter((t) => !t.needsLiveToggle || liveQuotesEnabled());
  const done = new Set();
  try {
    if (ghDispatchPat() && targets.length) {
      setMsg("수집 상태 확인 중…");
      const before = {};
      for (const t of targets) {
        try { before[t.path] = (await ghReadLiveFile(t)).updated || ""; } catch (e) { before[t.path] = ""; }
      }
      const allFresh = targets.every((t) => {
        const age = liveGlobalAgeHours(before[t.path]);
        return age != null && age * 60 < GH_DISPATCH_FRESH_MIN;
      });
      if (allFresh) {
        setMsg(`${GH_DISPATCH_FRESH_MIN}분 이내 수집분이라 그대로 사용합니다`);
      } else {
        setMsg("수집 실행 요청 중…");
        const failed = [];
        for (const t of targets) {
          try { await ghDispatchWorkflow(t.wf); } catch (err) { failed.push(`${t.label} ${err.message}`); }
        }
        if (failed.length === targets.length) {
          setMsg(`수집 실행 실패(${failed.join(" · ")}) — 저장된 값으로 표시합니다`);
        } else {
          const deadline = Date.now() + GH_DISPATCH_WAIT_MS;
          while (Date.now() < deadline && done.size < targets.length) {
            await sleepMs(GH_DISPATCH_POLL_MS);
            for (const t of targets) {
              if (done.has(t.path)) continue;
              try {
                const data = await ghReadLiveFile(t);
                // updated가 바뀌어야 "새로 수집됨"이다 — 같은 값이면 아직 커밋 전이다.
                if (data && data.updated && data.updated !== before[t.path] && data.prices) {
                  state[t.key] = { data, fetchedAt: Date.now() };
                  if (t.key === "liveKr") state.liveKrError = null;
                  done.add(t.path);
                }
              } catch (e) { /* 폴링 중 일시 오류는 다음 회차에서 다시 본다 */ }
            }
            const left = Math.max(0, Math.round((deadline - Date.now()) / 1000));
            if (done.size < targets.length) setMsg(`수집 대기 중… ${done.size}/${targets.length} 완료 · ${left}초 남음`);
          }
          if (done.size < targets.length) {
            setMsg(`일부 수집이 제시간에 끝나지 않았습니다(${done.size}/${targets.length}) — 잠시 뒤 다시 눌러 주세요`);
          }
        }
      }
    }
    // 새로 받은 계열은 state에 이미 넣었고, 나머지는 캐시를 비워 다시 읽게 한다.
    for (const t of GH_DISPATCH_TARGETS) if (!done.has(t.path)) state[t.key] = null;
    await renderMyAssets();
    if (done.size) {
      const stamps = GH_DISPATCH_TARGETS
        .filter((t) => state[t.key] && state[t.key].data)
        .map((t) => `${t.label} ${String(state[t.key].data.updated || "").slice(11)}`);
      setMsg(`수집 완료 — ${stamps.join(" · ")} 기준`);
    }
  } finally {
...
/* A99: 수집 시각("2026-09-21 12:57 KST")이 몇 시간 전인지. 날짜 문자열끼리 비교하면
   todayStr()이 UTC 기준이라 KST 새벽~오전에 멀쩡한 데이터를 "어제 것"으로 오판한다 —
   경과 시간으로 재는 편이 시간대에 안전하고 "몇 시간 전"이라는 정보까지 준다. */
function liveGlobalAgeHours(updated) {
  const m = /^(\d{4}-\d{2}-\d{2}) (\d{2}:\d{2}) KST$/.exec(String(updated || ""));
  if (!m) return null;
  const ts = Date.parse(`${m[1]}T${m[2]}:00+09:00`);
  return Number.isFinite(ts) ? (Date.now() - ts) / 3600000 : null;
}
...
function marketIndexBarHTML(liveGlobal) {
  ...
  // A99(2026-09-21 사용자 "추이 코스피반영 오류체크"): GitHub 예약 수집은 best-effort라
  // 지연·누락되면 며칠 전 값이 그대로 남는다(실사례: 월요일 낮에 금요일 종가가 찍혀 있었고,
  // 그 상태로 저장한 스냅샷에 과거 지수가 함께 기록됐다). 오래된 값이면 눈에 보이게 알린다.
  const age = liveGlobalAgeHours(snap.updated);
  const staleMsg = age != null && age >= 6
    ? `<br><span style="color:#c26a00;">⚠ 마지막 수집이 ${age >= 48 ? Math.floor(age / 24) + "일" : Math.floor(age) + "시간"} 전입니다 — 예약 수집이 지연·누락된 상태라, 지금 스냅샷을 저장하면 이 과거 지수가 함께 기록됩니다.</span>`
    : "";
  return `<p class="stat-sub">📈 코스피 ${k.price.toLocaleString(undefined, { maximumFractionDigits: 2 })} <span style="color:${color}">${signedPct(pct)}</span> — ${snap.updated} 기준(30분 주기 수집, 실시간 아님)${staleMsg}</p>`;
}
...
` });
  ...
  const clRows = changelog.slice(0, 60).map((e) => {
    const items = e.changes.map((c) => `${c.account || "미지정"} ${c.symbol} ${CHANGE_TYPE_LABEL[c.type] || c.type}(${changeDetailLabel(c)})`).join(", ");
...
async function renderMyAssets() {
  ...
  for (const it of items) {
    ...
    /* 목표 도달 "종목별 실적 반영" 옵션용 — 선택된 기간(1/3/6/12개월)의 실제 가격 등락률(연환산).
       A45(2026-08-12): 직접입력 모드에서도 최근 12개월 기준으로 계산한다. 종전엔 null이라
  ...
  const weightedTrailReturn = vsum > 0 ? wsum / vsum : null;
  ...
  // 수익률 분석(비중별)용 — 매입원가/월배당도 region·style별로 함께 집계(항상 전체 계좌 기준)
  ...
  const REGION_ORDER = ["한국", "미국", "글로벌", "미분류"];
  const STYLE_ORDER = ["성장", "배당", "안전", "개별주", "미분류"];
  // A84(2026-09-07 사용자 요청 "종합/일반계좌/ETF 3분할"): 시장·스타일·카테고리 비중을
  // "종합"(필터 없음) · "일반계좌"(계좌유형만) · "ETF"(자산유형만)로 각각 따로 필터링해 볼 수
  // 있게 한다. DC·연금·IRP는 세금이연 계좌라 당장 리밸런싱이 자유롭지 않으므로, 실제 처분
  // 가능한 비중은 일반계좌만 따로 봐야 한다는 요청 — accountTaxType()으로 계좌명을 일반화
  // 판정(A52 원칙 유지).
  function buildAllocCompositionHTML(rows) {
    const rMap = new Map(), sMap = new Map(), cMap = new Map();
    let tv = 0;
    for (const p of rows) {
      if (p.value <= 0) continue;
      tv += p.value;
      const region = (p.meta && p.meta.region) || "미분류";
      const style = (p.meta && p.meta.style) || "미분류";
      rMap.set(region, (rMap.get(region) || 0) + p.value);
      sMap.set(style, (sMap.get(style) || 0) + p.value);
      const cat = p.meta && p.meta.category ? p.meta.category : "기타";
      cMap.set(cat, (cMap.get(cat) || 0) + p.value);
    }
    const kpiHTML = tv > 0 ? `<div class="stat-row" style="margin-top:14px;">
      ${REGION_ORDER.filter((r) => rMap.has(r)).map((r) => `<div class="stat">
        <p class="stat-label">${r} 투자 비중</p>
        <p class="stat-value">${((rMap.get(r) / tv) * 100).toFixed(1)}%</p>
        <p class="stat-sub">${fmtW(rMap.get(r))}</p>
      </div>`).join("")}
    </div>` : `<p class="compare-empty">해당 범위의 보유 종목이 없습니다.</p>`;
    const styleHTML = STYLE_ORDER.filter((s) => sMap.has(s)).map((s, i) => {
      const v = sMap.get(s);
    ...
    const catHTML = [...cMap.entries()].sort((a, b) => b[1] - a[1]).map(([cat, v], i) => {
      const pct = tv > 0 ? (v / tv) * 100 : 0;
      return `<div class="bar-row">
    ...
    // A91(2026-09-08 사용자 요청 "버튼 누르면 top5 종목 비중 보여주기"): 지금 선택된 범위
    // (종합/일반계좌/ETF) 안에서 평가액 상위 5종목 비중 — details/summary라 클릭해야
    // 펼쳐지고, 별도 JS 배선 없이 이 함수 안에서 rows·tv를 그대로 재사용한다.
    // A92(2026-09-08 사용자 요청 "etf 글로벌 비중 top5종목"): TOP5 안에 지역(한국/미국/글로벌)
    // 하위필터를 추가 — 예: ETF 범위를 고른 뒤 "글로벌"을 누르면 ETF 중 해외상장분만 다시 랭킹한다.
    const availRegions = REGION_ORDER.filter((r) => rMap.has(r));
    const selRegion = availRegions.includes(state.myTop5Region) ? state.myTop5Region : "all";
    const top5Pool = selRegion === "all" ? rows : rows.filter((p) => ((p.meta && p.meta.region) || "미분류") === selRegion);
    const top5Rows = top5Pool.filter((p) => p.value > 0).sort((a, b) => b.value - a.value).slice(0, 5);
    const top5HTML = top5Rows.map((p, i) => {
      const pct = tv > 0 ? (p.value / tv) * 100 : 0;
      const name = p.meta ? p.meta.name : p.symbol;
      return `<div class="bar-row">
    ...
    return `${kpiHTML}
      ...
      <div class="bar-list">${styleHTML}</div>
      <p class="chart-title" style="margin-top:20px;">🧩 자산 성격별 비중 (카테고리)</p>
      <div class="bar-list">${catHTML}</div>
      <details id="top5DetailsBox" class="collapse-box" style="margin-top:20px;" ${state.myTop5Open ? "open" : ""}>
        <summary>🔝 TOP5 종목 비중 (눌러서 보기)</summary>
        <div class="collapse-body">
          ${top5RegionBtnsHTML}
          <div class="bar-list">${top5HTML || `<p class="compare-empty">해당 범위의 보유 종목이 없습니다.</p>`}</div>
        </div>
      </details>
  ...
  // 하나의 AND 조건으로 묶었던 이전 토글을 독립된 두 관점으로 분리한다 — "종합"은 필터 없음(둘 다
  // 포함), "일반계좌"는 계좌유형만, "ETF"는 자산유형만 거른다.
  const ALLOC_SCOPES = ["all", "general", "etf"];
  const filterAllocRows = (scope) => {
    if (scope === "general") return perRow.filter((p) => accountTaxType(p.account) === "general");
    if (scope === "etf") return perRow.filter((p) => !p.meta || p.meta.assetType !== "stock");
    return perRow;
  };
  const allocScope = ALLOC_SCOPES.includes(state.myAllocScope) ? state.myAllocScope : "all";
  const allocScopeHTML = buildAllocCompositionHTML(filterAllocRows(allocScope));
  ...
  /* A89(2026-09-07 사용자 요청): 세금 블록 전체를 접어둔다 — 세부 항목이 길어 월배당현황 탭을
     스크롤할 때마다 지나가야 했다. 다만 완전히 감추면 "지금 인출 가능"처럼 매일 보는 숫자까지
     묻히므로, 접힌 상태의 summary 줄에 핵심 3개(인출 가능·원천징수·종합과세 도달률)를 실어
     펼치지 않아도 읽히게 한다. 펼치면 계좌별·종목별 과세 상세까지 나온다(buildTaxDetailHTML). */
  const finPctNow = finThreshold > 0 ? (totalAnnualTaxableIncome / finThreshold) * 100 : 0;
  // A90(2026-09-07 사용자 요청): "🧾 세금현황" 탭으로 이동 — 더 이상 접어둘 필요 없다(탭
  // 자체가 안 눌러도 안 보이는 접힌 상태이므로 details 래퍼를 걷어냈다).
  const taxSummaryHTML = `
    ...
    ${buildTaxDetailHTML(perRow)}`;

  // A90: 연도별 배당 시뮬레이션(자사주·특별배당 반영) + 연금 절세 넛지 — 세금현황 탭 전용.
  const myShareGrants = readShareGrants();
  const mySpecialDivs = readSpecialDivs();
  const myGrantScenario = state.myGrantScenario === "low" ? "low" : "high";
  const taxProjectionHTML = buildTaxProjectionHTML(perRow, myShareGrants, mySpecialDivs, finThreshold, myGrantScenario);
  const shareGrantsHTML = buildShareGrantsHTML(myShareGrants);
  const specialDivHTML = buildSpecialDivHTML(mySpecialDivs);
  const pensionNudgeHTML = buildPensionNudgeHTML(accountMap, state.myBirthYear);

  // A90(2번째 요청 "총자산에도 자사주 반영 버튼"): 지금 보유가 아니라 "받을 예정"인 몫이므로
  // 기본은 끄고, 켰을 때만 현재가 기준 평가액에 더해 미래 전망치를 보여준다(qty 기준 —
  // 조건미달 시나리오는 세금현황 탭의 시나리오 토글에서만 다룬다, 여기선 항상 기본 수량).
  const includeGrantsInTotal = localStorage.getItem(MY_INCLUDE_GRANTS_KEY) === "1";
  const grantProjectedValue = myShareGrants.reduce((s, g) => {
    const hit = perRow.find((p) => p.symbol === g.symbol);
    return hit ? s + Number(g.qty || 0) * hit.close : s;
  }, 0);
  const grantProjectedQty = myShareGrants.reduce((s, g) => s + Number(g.qty || 0), 0);
  const displayTotalValue = includeGrantsInTotal ? totalValue + grantProjectedValue : totalValue;

  result.innerHTML = `
    <div class="stat-row" style="margin-top:14px;">
      <div class="stat stat-hero-card">
        <p class="stat-label">총 평가액${includeGrantsInTotal ? ' <span style="color:var(--good); font-weight:400; font-size:11px;">(자사주 예정분 포함)</span>' : ""}</p>
        <p class="stat-hero" style="font-size:28px;">${fmtW(displayTotalValue)}</p>
        ...
        ${myShareGrants.length ? `<label style="font-size:11px; display:flex; align-items:center; gap:4px; margin-top:4px; cursor:pointer;"><input type="checkbox" id="myIncludeGrantsToggle"${includeGrantsInTotal ? " checked" : ""}> 📈 자사주 예정분(${grantProjectedQty.toLocaleString()}주 · ${fmtW(grantProjectedValue)}) 반영</label>` : ""}
      </div>
      <div class="stat">
    ...
    <div class="dash-tabs" id="myDashTabs">
      ...
      <button type="button" class="dash-tab-btn" data-tab="tax">🧾 세금현황</button>
    ...
    <div class="dash-panel" data-tab="trend" hidden>
      ...
      <p class="chart-title" style="margin-top:24px;">📊 수익률 추이 (일별·월별·연도별)</p>
      <p class="stat-sub">매입단가 기준 수익률을 스냅샷 시점마다 함께 기록합니다 — 오늘부터 쌓이기 시작하며 과거로 소급되지 않습니다.</p>
      <div class="controls" style="margin-bottom:8px; flex-wrap:wrap; gap:8px 16px;">
        <select id="myReturnTrendGranularity" aria-label="수익률 추이 기간 단위">
          <option value="daily">일별</option>
          <option value="monthly">월별</option>
    ...
    <div class="dash-panel" data-tab="alloc" hidden>
      <div id="allocScopeBtns" class="seg" style="margin-bottom:8px;">
        <button type="button" data-scope="all">종합 (일반계좌+ETF)</button>
        <button type="button" data-scope="general">일반계좌</button>
        <button type="button" data-scope="etf">ETF</button>
      </div>
      <div id="myAllocScopeBody">${allocScopeHTML}</div>
    ...
    <div class="dash-panel" data-tab="heatmap" hidden>
      ...
      <div class="controls" style="margin:10px 0;">
        ...
        ${liveQuotesEnabled() ? `<button type="button" id="myHeatmapLiveNowBtn" class="btn-action">⏱️ 지금 확인</button>
        <span id="myHeatmapLiveNowStatus" class="action-status"></span>` : ""}
      </div>
    ...
    <div class="dash-panel" data-tab="tax" hidden>
      <p class="chart-title">🧾 세금 상세</p>
      ${taxSummaryHTML}

      <p class="chart-title" style="margin-top:24px;">📊 연도별 종합과세 한도 (자사주·특별배당 반영)</p>
      <div class="controls" style="margin-bottom:8px; flex-wrap:wrap; gap:8px 16px;">
        <div id="taxProjThresholdBtns" class="seg">
          <button type="button" data-threshold="20000000">2,000만원</button>
          <button type="button" data-threshold="10000000">1,000만원</button>
        </div>
        ${myShareGrants.some((g) => g.qtyLow != null && g.qtyLow !== "") ? `<div id="myGrantScenarioBtns" class="seg">
          <button type="button" data-scenario="high">조건 충족</button>
          <button type="button" data-scenario="low">조건 미달</button>
        </div>` : ""}
      </div>
      <div id="taxProjectionBody">${taxProjectionHTML}</div>

      <p class="chart-title" style="margin-top:24px;">🎁 자사주 수령 예정</p>
      <div id="myGrantsBody">${shareGrantsHTML}</div>

      <p class="chart-title" style="margin-top:24px;">💵 특별배당 추정</p>
      <div id="mySpecialDivBody">${specialDivHTML}</div>

      <p class="chart-title" style="margin-top:24px;">🏛️ 연금 절세 넛지</p>
      <div id="myPensionNudgeBody">${pensionNudgeHTML}</div>
    </div>
    ...
    <div class="dash-panel" data-tab="suff" hidden>
      ...
      <p class="chart-title" style="margin-top:24px;">🎯 목표 주식수 추적</p>
      ${goalTrackerHTML}
    </div>
    ...
    <div class="dash-panel" data-tab="settings" hidden>
      ...
      <p class="chart-title" style="margin-top:20px;">🔑 수집 실행 토큰 (선택 · 이 기기에만 저장)</p>
      <p class="stat-sub">여기에 GitHub 토큰을 저장하면 <b>"⏱️ 지금 확인"을 누를 때 수집 워크플로가 실제로 실행</b>되고, 새 시세가 올라올 때까지 기다렸다가 화면을 갱신합니다. 저장하지 않으면 종전처럼 <b>이미 수집된 값만 다시 읽습니다</b>(예약 수집이 멈춰 있으면 같은 과거 값이 계속 나옵니다).</p>
      <div class="action-row" style="margin:8px 0;">
        <input type="password" id="myGhPatInput" placeholder="github_pat_..." style="min-width:220px;" autocomplete="off" aria-label="GitHub 토큰">
        <button type="button" id="myGhPatSaveBtn" class="btn-action">저장</button>
        <button type="button" id="myGhPatClearBtn" class="btn-action">삭제</button>
        <span id="myGhPatStatus" class="action-status">${ghDispatchPat() ? "저장됨 ✓" : "미저장"}</span>
      </div>
      <p class="stat-sub">만드는 법: GitHub → Settings → Developer settings → <b>Fine-grained personal access token</b> → Repository access는 <code>14fiance</code> 하나만, Permissions는 <b>Actions: Read and write</b>만 주면 됩니다. ⚠️ 토큰은 <b>이 기기의 브라우저 저장소에만</b> 보관되며 내보내기 JSON·웹훅·저장소에는 절대 실리지 않습니다. 기기를 잃어버리면 GitHub에서 그 토큰을 폐기(revoke)하세요.</p>
  ...
  if (document.getElementById("myOutlookPastComboChart")) {
    ...
    const rateRangeBtns = document.getElementById("myRateRangeBtns");
    if (rateRangeBtns) {
      rateRangeBtns.querySelectorAll("button").forEach((b) => {
        b.addEventListener("click", () => { state.myRateRange = b.dataset.range; renderOutlookPastComboChart(); });
      });
    }
    ...
    (async () => {
      ...
      try {
        ...
        for (const p of PAST) {
          ...
          const cells = [20, 60, 200].map((n) => {
            const m = ma(n);
            if (m == null) return `<td>—</td>`;
            const gap = last / m - 1;
            return `<td style="color:${gap >= 0 ? "var(--good)" : "var(--critical)"};">${gap >= 0 ? "+" : ""}${(gap * 100).toFixed(1)}%<br><span class="stat-sub" style="font-size:11px;">${m.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span></td>`;
          }).join("");
          maRows.push(`<tr><td>${p.label}<br><span class="stat-sub" style="font-size:11px;">종가 ${last.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span></td>${cells}</tr>`);
        }
        if (series.length) {
        ...
        if (maEl) {
          maEl.innerHTML = `<div style="overflow-x:auto;"><table class="account-summary-table" style="font-size:12.5px;">
              <thead><tr><th>지수(프록시)</th><th>M20</th><th>M60</th><th>M200</th></tr></thead>
              <tbody>${maRows.join("")}</tbody>
            </table></div>
            <p class="stat-sub" style="margin-top:6px;">숫자는 <b>현재가가 이동평균보다 몇 % 위/아래인지</b>(이격도)이고 작은 글씨가 이동평균값입니다. 주간 수집 데이터라 최신 며칠은 반영이 늦을 수 있고, ETF 프록시 기준이라 지수 자체의 이평선과는 값이 다릅니다 — <b>추세 방향</b>을 보는 용도입니다. M200 위=장기 상승추세, M20이 M60·M200 아래로 내려가면 단기 조정 신호로 읽습니다.</p>`;
        }
      } catch (err) {
  ...
  if (indexOutlookBtn) {
    indexOutlookBtn.addEventListener("click", () => {
      ...
      // 방금 찍은 점이 전망 차트의 "실제" 선에도 바로 반영되게 그 차트만 다시 그린다
      // (2건째부터 선이 생김). 전체 재렌더는 시세 재조회까지 일으켜 과하다.
      renderOutlookFutureChart();
  ...
  // 대시보드 헤더에만 있어서 비중 히트맵 탭 안에서는 스크롤을 올려야만 눌렀다 — 히트맵
  // 탭에도 같은 동작(3분 캐시 무시하고 최신시세 강제 재조회)의 버튼을 하나 더 둔다.
  const heatmapNowBtn = document.getElementById("myHeatmapLiveNowBtn");
  if (heatmapNowBtn) {
    // A100: 헤더 버튼과 동일하게 — 토큰이 있으면 수집까지 실행하고 기다린다.
    heatmapNowBtn.addEventListener("click", () => refreshLiveQuotesNow("myHeatmapLiveNowBtn", "myHeatmapLiveNowStatus"));
  }

  // A84(2026-09-07 사용자 요청 "종합/일반계좌/ETF 3분할"): 비중분석 탭 종합/일반계좌/ETF
  // 토글 — 시세 재조회 없이 #myAllocScopeBody만 다시 그린다(finIncomeThresholdBtns와 같은 패턴).
  const allocScopeBtns = document.getElementById("allocScopeBtns");
  if (allocScopeBtns) {
    const renderAllocScope = () => {
      const body = document.getElementById("myAllocScopeBody");
      const scope = ALLOC_SCOPES.includes(state.myAllocScope) ? state.myAllocScope : "all";
      if (body) body.innerHTML = buildAllocCompositionHTML(filterAllocRows(scope));
      allocScopeBtns.querySelectorAll("button").forEach((b) => {
        b.classList.toggle("active", b.dataset.scope === scope);
      });
      // A92: TOP5 지역 하위필터 버튼은 myAllocScopeBody가 통째로 다시 그려질 때마다 새로
      // 생기므로 매번 재배선한다(scope 토글 재사용).
      const regionBtns = body ? body.querySelector("#top5RegionBtns") : null;
      if (regionBtns) {
      ...
      // 안 그러면 지역필터를 누를 때마다 방금 펼친 TOP5 박스가 도로 접혀 결과가 안 보인다.
      const top5Details = body ? body.querySelector("#top5DetailsBox") : null;
      if (top5Details) top5Details.addEventListener("toggle", () => { state.myTop5Open = top5Details.open; });
      const rankDetails = body ? body.querySelector("#rankDetailsBox") : null;
      if (rankDetails) rankDetails.addEventListener("toggle", () => { state.myRankOpen = rankDetails.open; });
    };
    allocScopeBtns.querySelectorAll("button").forEach((b) => {
      b.addEventListener("click", () => { state.myAllocScope = b.dataset.scope; renderAllocScope(); });
    });
    renderAllocScope();
  }

  // A74/A90(2026-09-07 확장): 금융소득종합과세 기준액 2,000만원/1,000만원 토글 — 월배당현황의
  // finIncomeBody와 세금현황 탭의 taxProjectionBody가 같은 state.myFinIncomeThreshold를 공유하므로,
  // 어느 쪽 버튼을 눌러도 둘 다 함께 갱신해 서로 어긋나지 않게 한다. 시세 재조회 없이 두 body만 다시 그린다.
  const finIncomeThresholdBtns = document.getElementById("finIncomeThresholdBtns");
  const taxProjThresholdBtns = document.getElementById("taxProjThresholdBtns");
  const myGrantScenarioBtns = document.getElementById("myGrantScenarioBtns");
  if (finIncomeThresholdBtns || taxProjThresholdBtns || myGrantScenarioBtns) {
    ...
    const renderTaxProjection = () => {
      const body = document.getElementById("taxProjectionBody");
      const threshold = state.myFinIncomeThreshold || 20000000;
      const scenario = state.myGrantScenario === "low" ? "low" : "high";
      if (body) body.innerHTML = buildTaxProjectionHTML(perRow, myShareGrants, mySpecialDivs, threshold, scenario);
    };
    const syncToggleUI = () => {
      const threshold = state.myFinIncomeThreshold || 20000000;
      const scenario = state.myGrantScenario === "low" ? "low" : "high";
      [finIncomeThresholdBtns, taxProjThresholdBtns].forEach((el) => {
        if (!el) return;
        el.querySelectorAll("button").forEach((b) => b.classList.toggle("active", Number(b.dataset.threshold) === threshold));
      });
      if (myGrantScenarioBtns) {
        myGrantScenarioBtns.querySelectorAll("button").forEach((b) => b.classList.toggle("active", b.dataset.scenario === scenario));
      }
    };
    [finIncomeThresholdBtns, taxProjThresholdBtns].forEach((el) => {
      if (!el) return;
      el.querySelectorAll("button").forEach((b) => {
        b.addEventListener("click", () => {
          state.myFinIncomeThreshold = Number(b.dataset.threshold);
          renderFinIncome(); renderTaxProjection(); syncToggleUI();
        });
      });
    });
    if (myGrantScenarioBtns) {
      myGrantScenarioBtns.querySelectorAll("button").forEach((b) => {
        b.addEventListener("click", () => { state.myGrantScenario = b.dataset.scenario; renderTaxProjection(); syncToggleUI(); });
      });
    }
    renderFinIncome(); renderTaxProjection(); syncToggleUI();
  }

  // A90: 자사주 수령 예정 편집표 — 필드 하나 바뀔 때마다 배열을 통째로 다시 써서 저장하고
  // (myContribRows와 같은 결의 패턴), 총자산·연도별 시뮬레이션 둘 다에 영향을 주므로 전체를
  // 다시 그린다(saveMyAssets + renderMyAssets — 목록이 작아 비용이 크지 않다).
  const myGrantsTable = document.getElementById("myGrantsTable");
  if (myGrantsTable) {
    myGrantsTable.querySelectorAll(".grant-field").forEach((el) => {
      el.addEventListener("change", () => {
        const idx = Number(el.closest("tr").dataset.idx);
        const list = readShareGrants();
        if (!list[idx]) return;
        const v = el.value;
        list[idx][el.dataset.field] = el.type === "number" ? (v === "" ? "" : Number(v)) : v;
        localStorage.setItem(MY_SHARE_GRANTS_KEY, JSON.stringify(list));
        saveMyAssets();
        renderMyAssets();
      });
    });
    myGrantsTable.querySelectorAll(".grant-del-btn").forEach((btn) => {
  ...
  const myGrantAddBtn = document.getElementById("myGrantAddBtn");
  if (myGrantAddBtn) {
    myGrantAddBtn.addEventListener("click", () => {
      const list = readShareGrants();
      list.push({ symbol: "", account: "", month: "", qty: "", qtyLow: "" });
      localStorage.setItem(MY_SHARE_GRANTS_KEY, JSON.stringify(list));
      saveMyAssets();
      renderMyAssets();
    });
  ...
  // A90: 특별배당 추정 편집표 — 자사주 표와 동일 패턴
  const mySpecialDivTable = document.getElementById("mySpecialDivTable");
  if (mySpecialDivTable) {
    mySpecialDivTable.querySelectorAll(".special-field").forEach((el) => {
      el.addEventListener("change", () => {
        const idx = Number(el.closest("tr").dataset.idx);
        const list = readSpecialDivs();
        if (!list[idx]) return;
        const v = el.value;
        list[idx][el.dataset.field] = el.type === "number" ? (v === "" ? "" : Number(v)) : v;
        localStorage.setItem(MY_SPECIAL_DIV_KEY, JSON.stringify(list));
        saveMyAssets();
        renderMyAssets();
      });
    });
    mySpecialDivTable.querySelectorAll(".special-del-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        const idx = Number(btn.dataset.idx);
        const list = readSpecialDivs();
        list.splice(idx, 1);
  ...
  const mySpecialAddBtn = document.getElementById("mySpecialAddBtn");
  if (mySpecialAddBtn) {
    mySpecialAddBtn.addEventListener("click", () => {
      const list = readSpecialDivs();
      list.push({ symbol: "", year: new Date().getFullYear(), dps: "", recordMonth: "", note: "" });
      localStorage.setItem(MY_SPECIAL_DIV_KEY, JSON.stringify(list));
      saveMyAssets();
      renderMyAssets();
    });
  ...
  // A90: 연금 절세 넛지의 출생연도 입력 — 시세 재조회 없이 넛지 블록만 다시 그린다
  const myBirthYearInput = document.getElementById("myBirthYear");
  if (myBirthYearInput) {
    myBirthYearInput.addEventListener("change", () => {
      state.myBirthYear = myBirthYearInput.value;
      saveMyAssets();
      const body = document.getElementById("myPensionNudgeBody");
      if (body) body.innerHTML = buildPensionNudgeHTML(accountMap, state.myBirthYear);
    });
  }

  // A90: "총 평가액"에 자사주 예정분을 얹어보는 토글 — 총자산 전체가 바뀌므로 전체 재렌더
  const myIncludeGrantsToggle = document.getElementById("myIncludeGrantsToggle");
  if (myIncludeGrantsToggle) {
    myIncludeGrantsToggle.addEventListener("change", () => {
      localStorage.setItem(MY_INCLUDE_GRANTS_KEY, myIncludeGrantsToggle.checked ? "1" : "0");
      renderMyAssets();
    });
  }
  ...
  // A100: 수집 실행 토큰 — 저장/삭제. 값 자체는 화면에 다시 뿌리지 않는다(어깨너머 노출 방지).
  const ghPatSaveBtn = document.getElementById("myGhPatSaveBtn");
  if (ghPatSaveBtn) {
    ghPatSaveBtn.addEventListener("click", () => {
      const input = document.getElementById("myGhPatInput");
      const v = (input.value || "").trim();
      if (!v) { flashStatus("myGhPatStatus", "토큰을 입력하세요"); return; }
      localStorage.setItem(MY_GH_PAT_KEY, v);
      input.value = "";
      flashStatus("myGhPatStatus", "저장됨 ✓ — 이 기기에만 보관됩니다", 4000);
    });
  }
  const ghPatClearBtn = document.getElementById("myGhPatClearBtn");
  if (ghPatClearBtn) {
    ghPatClearBtn.addEventListener("click", () => {
      localStorage.removeItem(MY_GH_PAT_KEY);
      const input = document.getElementById("myGhPatInput");
      if (input) input.value = "";
      flashStatus("myGhPatStatus", "삭제됨 — GitHub에서도 폐기(revoke)하세요", 4000);
    });
  }
  ...
  // A44: 🎯 목표 주식수 추적 — 종목 선택→추가, 목표수량 입력, 삭제
  const goalAddBtn = document.getElementById("myGoalAddBtn");
  if (goalAddBtn) {
    goalAddBtn.addEventListener("click", () => {
      const sym = document.getElementById("myGoalSymbolPick").value;
      if (!sym) return;
      const goals = loadMyGoals();
      if (!goals.some((g) => g.symbol === sym)) {
        goals.push({ symbol: sym, targetQty: 0 });
        saveMyGoals(goals);
        renderMyAssets();
      }
    });
  }
  document.querySelectorAll(".my-goal-target").forEach((el) => {
    el.addEventListener("change", () => {
      const goals = loadMyGoals();
      const g = goals.find((x) => x.symbol === el.dataset.symbol);
      if (g) { g.targetQty = parseFloat(el.value) || 0; saveMyGoals(goals); renderMyAssets(); }
    });
  });
  ...
  // A46: 수익률 추이 — 범위(총계좌/계좌별/비중별/국가별) + 기간(일/월/연) select, 선택은 state에 보존.
  const rtSel = document.getElementById("myReturnTrendScope");
  const rtGranSel = document.getElementById("myReturnTrendGranularity");
  if (rtSel && rtGranSel) {
    const rtRegions = [...new Set(perRow.map((p) => (p.meta && p.meta.region) || "미분류"))];
    const rtStyles = [...new Set(perRow.map((p) => (p.meta && p.meta.style) || "미분류"))];
    rtSel.innerHTML = `<option value="total">총계좌(전체)</option>` +
      `<optgroup label="계좌별">${[...accountMap.keys()].map((a) => `<option value="${a}">${a}</option>`).join("")}</optgroup>` +
      `<optgroup label="비중별(성향)">${rtStyles.map((s) => `<option value="${s}">${s}</option>`).join("")}</optgroup>` +
      `<optgroup label="국가별(지역)">${rtRegions.map((r) => `<option value="${r}">${r}</option>`).join("")}</optgroup>`;
    const rtValidScopes = ["total", ...accountMap.keys(), ...rtStyles, ...rtRegions];
    rtSel.value = state.myReturnTrendScope && rtValidScopes.includes(state.myReturnTrendScope) ? state.myReturnTrendScope : "total";
    if (state.myReturnTrendGranularity) rtGranSel.value = state.myReturnTrendGranularity;
    const renderReturnTrend = () => {
      const body = document.getElementById("myReturnTrendBody");
      if (!body) return;
      const scope = rtSel.value;
      const scopeLabel = scope === "total" ? "총계좌" : scope;
      body.innerHTML = buildReturnTrendHTML(dailyHistory, history, rtGranSel.value, scope, scopeLabel);
    };
    rtSel.addEventListener("change", () => { state.myReturnTrendScope = rtSel.value; renderReturnTrend(); });
    rtGranSel.addEventListener("change", () => { state.myReturnTrendGranularity = rtGranSel.value; renderReturnTrend(); });
    renderReturnTrend();
  }
