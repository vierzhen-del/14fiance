...
/* ---------- A28: 종합 탭 "리포트 생성" ----------
   종합(계좌별 요약)·비중(성향별)·추이(MDD)·배당(TOP5) 정보를 한 텍스트로 모아 다운로드·
   클립보드·텔레그램·옵시디안에 공통으로 쓴다. 계좌명은 maskAccountLabel로 가린다 —
   텔레그램·클립보드로 이 텍스트가 그대로 밖에 나갈 수 있는 경로이기 때문(A26a와 같은 이유). */
function buildReportText(csv, history) {
  if (!csv || !csv.perRow || !csv.perRow.length) return null;
  const fmtW = (v) => fmtPrice(v, "KRW");
  const lines = [];
  lines.push(`📊 14fiance 포트폴리오 리포트 (${todayStr()})`);
  lines.push("");

  lines.push("[종합]");
  lines.push(`총 평가액: ${fmtW(csv.totalValue)}`);
  lines.push(`총 월배당: ${fmtW(csv.totalMonthlyDiv)}`);
  if (csv.totalProfit != null) lines.push(`총 손익(매입단가 입력분 기준): ${csv.totalProfit >= 0 ? "+" : ""}${fmtW(csv.totalProfit)}`);
  lines.push("");

  lines.push("[계좌별 요약]");
  for (const [acc, g] of csv.accountMap) {
    const pct = csv.totalValue > 0 ? (g.value / csv.totalValue) * 100 : 0;
    lines.push(`- ${maskAccountLabel(acc)}: ${fmtW(g.value)} (${pct.toFixed(1)}%), 월배당 ${fmtW(g.monthlyDiv)}`);
  }
  lines.push("");
  ...
  for (const [s, v] of [...styleMap.entries()].sort((a, b) => b[1] - a[1])) {
    lines.push(`- ${s}: ${csv.totalValue > 0 ? ((v / csv.totalValue) * 100).toFixed(1) : "0.0"}% (${fmtW(v)})`);
  }
  lines.push("");
  ...
  if (mdd) {
    lines.push(`전체기간 MDD: ${(mdd.mdd * 100).toFixed(1)}% (${mdd.peakDate} → ${mdd.troughDate}, ${mdd.recoveredDate ? "회복 완료" : "미회복"})`);
    lines.push("※ 현재 보유 비중으로 과거 가격이력을 합성한 낙폭 — 실제 계좌 평가액 낙폭과는 다릅니다.");
  } else {
  ...
  if (history && history.length >= 2) {
    ...
    const diff = last.value - first.value;
    const pct = first.value > 0 ? (diff / first.value) * 100 : 0;
    lines.push(`평가액 변동(${first.month}→${last.month}): ${diff >= 0 ? "+" : ""}${fmtW(diff)} (${pct.toFixed(1)}%, 스냅샷 단순비교)`);
  }
  lines.push("");
  ...
  lines.push("[월배당 TOP5]");
  if (top5.length) {
    for (const p of top5) lines.push(`- ${(p.meta && p.meta.name) || p.symbol} (${maskAccountLabel(p.account || "계좌 미지정")}): ${fmtW(p.monthlyDiv)}/월`);
  } else {
