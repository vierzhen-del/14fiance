...
/* A76(2026-09-01 사용자 요청 — 인스타 인포그래픽 "연간 배당 히스토리" 참조), A77(같은 날
   사용자 추가 요청 "월간도 추가 버튼추가"): 연도별 또는 월별로 ①예상배당(회색, 그 시점
   스냅샷 예상액 합계) ②확정배당(일반배당/커버드콜 스택)을 나란히 보여준다. mode="year"면
   연도 단위로 12개월을 합산, mode="month"면 달 단위로 그대로 그린다(최근 24개월 캡).
   확정배당의 색상 구분은 divSplit(카테고리 비율, upsertMonthlySnapshot에서 스냅샷 시점에
   저장)이 있는 달만 실측 비율로 나눈다 — 없는 달(이 기능 이전 과거 이력)은 "구분 데이터
   없음" 회색 세그먼트로 남겨 값을 지어내지 않는다(2026-09-01 사용자 확인: "지금부터 새로
   기록 시작"). */
function buildDivHistoryBarsHTML(divHistory, snapshotHistory, mode) {
  const dh = divHistory || {};
  ...
  if (mode === "month") {
    ...
    units = sortedMonths.map((key) => {
      ...
      const est = snap && snap.monthlyDiv > 0 ? snap.monthlyDiv : 0;
      const confirmed = dh[key] > 0 ? dh[key] : 0;
      let splitGeneral = 0, splitCC = 0, splitKnown = 0;
      if (confirmed > 0 && snap && snap.divSplit) {
        splitGeneral = confirmed * snap.divSplit.generalPct;
        splitCC = confirmed * snap.divSplit.coveredCallPct;
        splitKnown = confirmed;
      }
      return { label: key.slice(2).replace("-", "."), est, confirmed, splitGeneral, splitCC, splitUnknown: confirmed - splitKnown };
    });
  } else {
    const years = new Set();
    ...
    units = sortedYears.map((y) => {
      ...
      return { label: y, est, confirmed, splitGeneral, splitCC, splitUnknown: confirmed - splitKnown };
    });
  }
  ...
  const groups = units.map(({ label, est, confirmed, splitGeneral, splitCC, splitUnknown }) => {
  ...
  const hasUnknown = units.some((r) => r.splitUnknown > 0);
  return `<div class="ydb-row">${groups}</div>
    <p class="stat-sub">${truncNote}예상배당(회색)은 그 ${mode === "month" ? "달" : "해"} 스냅샷 시점 예상액 합계, 확정배당(오른쪽 막대)은 일반배당/커버드콜로 스택. ${hasUnknown ? "2026-09-01 이전 확정 이력은 종목별 카테고리 기록이 없어 구분 없이 회색으로 표시됩니다(지어내지 않음) — 이후 스냅샷부터 자동으로 색이 채워집니다." : ""}</p>`;
}

/* A77: 연간/월간 전환 버튼 + 범례 + 실제 막대 컨테이너를 감싸는 섹션. 버튼 클릭은
   renderMyAssets() 재실행 없이 #divHistoryBody만 다시 그린다(mySignalSigmaWin류 패턴). */
function buildDivHistorySectionHTML(divHistory, snapshotHistory) {
  const yearHTML = buildDivHistoryBarsHTML(divHistory, snapshotHistory, "year");
  if (!yearHTML) return "";
...
async function renderMyAssets() {
  ...
  const annualDivHTML = buildDivHistorySectionHTML(state.myAssetsDivHistory || {}, history);
  ...
  if (divHistoryModeBtns) {
    const renderDivHistoryMode = () => {
      ...
      if (body) body.innerHTML = buildDivHistoryBarsHTML(state.myAssetsDivHistory || {}, history, mode);
