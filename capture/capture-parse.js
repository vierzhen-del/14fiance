...
- 현재가(있으면, 없으면 null)
- 수익률(%)(화면에 손익률·수익률이 퍼센트로 표시돼 있으면 그 숫자, 없으면 null — 부호(+/-)까지 그대로: 손실이면 음수로 적을 것)
- 평가손익(원)(화면에 "평가손익"·"손익금"처럼 금액으로 표시돼 있으면 그 숫자, 없으면 null — 부호(+/-)까지 그대로: 손실이면 음수로 적을 것)
장 하단/상단에 "총 평가금액", "자산" 등 계좌 합계가 보이면 별도 필드로 추출.
일부 증권사 앱(예: KB증권 계좌 잔고 화면)은 매입단가를 아예 표시하지 않고 손익률(%)·평가손익(원)만 보여준다 — 이 경우 매입단가는 null로 두고 대신 손익률·평가손익을 반드시 채울 것(둘 중 하나라도 화면에 보이면 놓치지 말 것). 매입단가가 안 보인다고 이 두 필드까지 건너뛰지 말 것.
...
스크린샷에 없는 값은 절대 추정하지 말고 null로 둘 것. 특히 보유수량(qty)은 평가금액÷현재가 등으로 역산해서 채우지 말 것 — 화면에 숫자가 안 보이면 반드시 null. 수익률(%)·평가손익(원)도 매입단가·현재가로 계산해서 채우지 말 것 — 화면에 그 숫자가 직접 보일 때만 적을 것.
...
{"account_label":"화면에 보이는 대표 계좌명(없으면 null, 여러 계좌가 섞여 있으면 첫 계좌명)","page_count":이미지수,"holdings":[{"page":1,"account":"이 홀딩의 계좌명(섞여 있을 때만 채우고, 한 계좌뿐이면 null 가능)","name":"...","symbol":"...","qty":0,"avgPrice":null,"currentPrice":null,"evalAmount":0,"returnPct":null,"profitAmount":null}],"reported_total":숫자 또는 null}`;
...
/* ---------- 교차검증 엔진 (이번 세션에서 실제로 오류를 잡아낸 패턴 재구현) ----------
   ...
   4) 매입단가 역산: 일부 증권사 앱(KB증권 계좌 잔고 등, 2026-08-12 실측)은 매입단가를 아예
      표시하지 않고 손익률(%)·평가손익(원)만 보여준다. avgPrice가 없을 때 profitAmount(우선,
      화면에 찍힌 절대금액이라 반올림 오차가 적다) → returnPct(차선) 순으로 매입원가를 역산한다:
      cost = evalAmount - profitAmount, 또는 cost = evalAmount ÷ (1 + returnPct/100).
   5) 수익률(%) 대조: (현재가-매입단가)÷매입단가 vs 화면에 직접 표시된 수익률(%) —
      매입단가 자릿수·소수점 오류(달러 종목 1000배 오류 등)를 잡아내기 위한 용도.
      화면 표시값은 실시간가 기준이라 우리 currentPrice(수집 종가)와는 소폭 어긋날 수 있어
      오차 허용폭을 행 오차(1%)보다 넉넉하게(3%p) 둔다. */
function validateAccountCapture(parsed) {
  const holdings = (parsed.holdings || []).map((h) => {
    ...
    if (!(h.avgPrice > 0) && h.qty > 0 && h.evalAmount > 0) {
      let cost = null, srcLabel = null;
      if (h.profitAmount != null) { cost = h.evalAmount - h.profitAmount; srcLabel = "평가손익"; }
      else if (h.returnPct != null) { cost = h.evalAmount / (1 + h.returnPct / 100); srcLabel = "수익률"; }
      if (cost != null && cost > 0) {
        h.avgPrice = Math.round((cost / h.qty) * 100) / 100;
        issues.push(`매입단가 화면 미표시 — ${srcLabel} 기준 역산(매입단가 ${h.avgPrice.toLocaleString()})`);
      }
    }
    if (h.currentPrice == null && h.qty > 0 && h.evalAmount > 0) h.currentPrice = h.evalAmount / h.qty;
    if (h.avgPrice > 0 && h.currentPrice != null && h.returnPct != null) {
      const calcPct = ((h.currentPrice - h.avgPrice) / h.avgPrice) * 100;
      const diffPt = Math.abs(calcPct - h.returnPct);
      if (diffPt > 3) issues.push(`수익률 불일치 (계산 ${calcPct.toFixed(1)}% vs 화면표시 ${h.returnPct.toFixed(1)}% — 매입단가 오류 가능성)`);
    }
    ...
    // A26a: 계좌 매칭은 마스킹 **전** 원본으로 해야 한다(숫자를 가린 뒤 매칭하면 계좌 자동채움이 깨진다).
    // 매칭이 끝난 뒤에야 마스킹해서 내보낸다 — 검토표·SOP 요약·이력이 전부 이 값을 물려받는다.
    const matchedAccount = matchAccountByLabel(rawAccountLabel);
    return { ...h, account: maskAccountLabel(h.account), issues, matchedSymbol: match ? match.symbol : null, matchedName: match ? match.name : null, rawAccountLabel: maskAccountLabel(rawAccountLabel), matchedAccount };
  });
  ...
  return { holdings, totalCheck, accountLabel: maskAccountLabel(parsed.account_label || null) };
}
...
function validateBuyPlanCapture(parsed) {
  const holdings = (parsed.holdings || []).map((h) => {
    ...
    const matchedAccount = matchAccountByLabel(rawAccountLabel); // A26a: 매칭은 원본, 표시는 마스킹
    return { ...h, account: maskAccountLabel(h.account), issues, matchedSymbol: match ? match.symbol : null, matchedName: match ? match.name : null, rawAccountLabel: maskAccountLabel(rawAccountLabel), matchedAccount };
  });
