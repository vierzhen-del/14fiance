...
스크린샷에 없는 값은 절대 추정하지 말고 null로 둘 것. 특히 보유수량(qty)은 평가금액÷현재가 등으로 역산해서 채우지 말 것 — 화면에 숫자가 안 보이면 반드시 null. 수익률(%)도 매입단가·현재가로 계산해서 채우지 말 것 — 화면에 퍼센트 숫자가 직접 보일 때만 적을 것.
...
{"account_label":"화면에 보이는 대표 계좌명(없으면 null, 여러 계좌가 섞여 있으면 첫 계좌명)","page_count":이미지수,"holdings":[{"page":1,"account":"이 홀딩의 계좌명(섞여 있을 때만 채우고, 한 계좌뿐이면 null 가능)","name":"...","symbol":"...","qty":0,"avgPrice":null,"currentPrice":null,"evalAmount":0,"returnPct":null}],"reported_total":숫자 또는 null}`;
...
/* ---------- 교차검증 엔진 (이번 세션에서 실제로 오류를 잡아낸 패턴 재구현) ----------
   ...
   4) 수익률(%) 대조: (현재가-매입단가)÷매입단가 vs 화면에 직접 표시된 수익률(%) —
